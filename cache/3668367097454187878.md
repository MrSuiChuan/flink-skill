...
> Any measurement process in and of itself inevitably affects the subject of measurement (see the [double-split experiment](<https://en.wikipedia.org/wiki/Double-slit_experiment#Relational_interpretation>)). Sampling CPU stack traces is no exception. In order to prevent unintended impacts on production environments, Flame Graphs are currently available as an opt-in feature. To enable it, you’ll need to set [`rest.flamegraph.enabled: true`](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#rest-flamegraph-enabled>) in [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file>). We recommend enabling it in development and pre-production environments, but you should treat it as an experimental feature in production.
...
Flame Graph construction is performed at the level of an individual [operator](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/concepts/glossary/#operator>) by default, i.e. all [task](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/concepts/glossary/#task>) threads of that operator are sampled in parallel and their stack traces are combined. If a method call consumes 100% of the resources in one of the parallel tasks but none in the others, the bottleneck might be obscured by being averaged out.
