...
> Amazon OpenSearch implements `CollectionManageableVectorStore`, enabling [Long-Term Memory](<//nightlies.apache.org/flink/flink-agents-docs-release-0.3/docs/development/memory/long_term_memory/>) support. Collections map to OpenSearch indices. OpenSearch indices do not natively support attaching arbitrary metadata, so any `metadata` passed to `createCollectionIfNotExists` is ignored. Callers needing per-document attributes should put them on the documents themselves.
...
> Amazon S3 Vectors does **not** implement `CollectionManageableVectorStore`, so it does not support [Long-Term Memory](<//nightlies.apache.org/flink/flink-agents-docs-release-0.3/docs/development/memory/long_term_memory/>) features. It also does not support `size()` or get-all operations: explicit document IDs are required for `get()` and `delete()`.
...
> `Mem0VectorStore` implements `CollectionManageableVectorStore`, enabling [Long-Term Memory](<//nightlies.apache.org/flink/flink-agents-docs-release-0.3/docs/development/memory/long_term_memory/>) support. Filters use the unified equality-only Filter DSL and are forwarded to the underlying Mem0 backend unchanged.
...
> **Limitations:**
>
>   * Cross-language resources are currently supported only when [running in Flink](<//nightlies.apache.org/flink/flink-agents-docs-release-0.3/docs/operations/deployment/#run-in-flink>), not in local development mode
