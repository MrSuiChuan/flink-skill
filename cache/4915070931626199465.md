...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/table/procedures/>).
...
Table（类似于 SQL 标准）是一种强类型的 API。 因此，存储过程的参数和返回类型都必须映射到 [data type](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/data-types/>)。
...
有关可以隐式映射到数据类型的类的完整列表, 请参阅[data type extraction section](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/data-types/#data-type-extraction>)。
...
在调用存储过程时，可以使用参数名称来指定参数值。命名参数允许同时传递参数名和值给存储过程，避免了因为错误的参数顺序而导致混淆，并提高了代码的可读性和可维护性。 此外，命名参数还可以省略非必需的参数，默认情况下会使用 `null` 进行填充。 我们可以通过 `@ArgumentHint` 注解来指定参数的名称，类型，是否是必需的参数等。
