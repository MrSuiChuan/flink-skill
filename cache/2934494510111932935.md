...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/sql/reference/utility/show/>).
...
可以在 [SQL CLI](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/interfaces/sql-client/>) 中执行 SHOW 语句。
...
**LIKE** 根据可选的 `LIKE` 语句与 `<sql_like_pattern>` 是否模糊匹配的所有 catalog。
...
**LIKE** Show all views with given view name and optional `LIKE` clause, whose name is whether similar to the `<sql_like_pattern>`.
...
**LIKE** Show all materialized tables with given materialized table name and optional `LIKE` clause, whose name is similar to the `<sql_like_pattern>`.
...
展示所有通过 [`ADD JAR`](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/utility/jar/#add-jar>) 语句加入到 session classloader 中的 jar。

Attention 当前 SHOW JARS 命令只能在 [SQL CLI](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/interfaces/sql-client/>) 或者 [SQL Gateway](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/interfaces/sql-gateway/overview/>) 中使用.
...
Attention 当前 SHOW JOBS 命令只能在 [SQL CLI](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/interfaces/sql-client/>) 或者 [SQL Gateway](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/interfaces/sql-gateway/overview/>) 中使用.
...
展示指定 catalog 和 database 下的所有模型。 如果没有指定 catalog 和 database，则将使用当前 catalog 和当前 database。另外可以用 `<sql_like_pattern>` 来过滤要返回的模型。

**LIKE** 根据可选的 `LIKE` 语句与 `<sql_like_pattern>` 是否模糊匹配的所有模型。
