...
These options are only necessary for _standalone_ application- or session deployments ([simple standalone](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/resource-providers/standalone/overview/>) or [Kubernetes](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/resource-providers/standalone/kubernetes/>)).

If you use Flink with [Yarn](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/resource-providers/yarn/>) or the [_active_ Kubernetes integration](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/resource-providers/native_kubernetes/>), the hostnames and ports are automatically discovered.
  ...
  * The `jobmanager.rpc.address` (defaults to _“localhost”_) and `jobmanager.rpc.port` (defaults to _6123_) config entries are used by the TaskManager to connect to the JobManager/ResourceManager. Set this to the hostname where the JobManager runs, or to the hostname of the (Kubernetes internal) service for the JobManager. This option is ignored on [setups with high-availability](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/ha/overview/>) where the leader election mechanism is used to discover this automatically.
...
**Parallelism**
  ...
  * `taskmanager.numberOfTaskSlots`: The number of slots that a TaskManager offers _(default: 1)_. Each slot can take one task or pipeline. Having multiple slots in a TaskManager can help amortize certain constant overheads (of the JVM, application libraries, or network connections) across parallel tasks or pipelines. See the [Task Slots and Resources](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/concepts/flink-architecture/#task-slots-and-resources>) concepts section for details.
...
Accepted values are:
  ...
  * `fixed-delay`, `fixeddelay`: Fixed delay restart strategy. More details can be found [here](<//nightlies.apache.org/flink/flink-docs-release-2.2//docs/ops/state/task_failure_recovery#fixed-delay-restart-strategy>).
  * `failure-rate`, `failurerate`: Failure rate restart strategy. More details can be found [here](<//nightlies.apache.org/flink/flink-docs-release-2.2//docs/ops/state/task_failure_recovery#failure-rate-restart-strategy>).
  * `exponential-delay`, `exponentialdelay`: Exponential delay restart strategy. More details can be found [here](<//nightlies.apache.org/flink/flink-docs-release-2.2//docs/ops/state/task_failure_recovery#exponential-delay-restart-strategy>).
...
For a detailed explanation of how these options interact, see the documentation on [TaskManager](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/memory/mem_setup_tm/>) and [JobManager](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/memory/mem_setup_jobmanager/>) memory configurations.
...
Flink’s network connections can be secured via SSL. Please refer to the [SSL Setup Docs](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/security/security-ssl/>) for detailed setup guide and background.
...
Flink has a pluggable authentication protocol agnostic delegation token framework. Please refer to the [Flink and Delegation Token Docs](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/security/security-delegation-token/>) for further details.
...
Please refer to the [Flink and Kerberos Docs](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/security/security-kerberos/>) for a setup guide and a list of external system to which Flink can authenticate itself via Kerberos.
...
Note that is not always necessary to integrate Flink with the resource orchestration framework. For example, you can easily deploy Flink applications on Kubernetes without Flink knowing that it runs on Kubernetes (and without specifying any of the Kubernetes config options here.) See [this setup guide](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/resource-providers/standalone/kubernetes/>) for an example.
...
Please refer to the [State Backend Documentation](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/>) for background on State Backends.
...
Please refer to the [metrics system documentation](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/metrics/>) for background on Flink’s metrics infrastructure.
...
Please refer to the [tracing system documentation](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/traces/>) for background on Flink’s tracing infrastructure.
...
See the [History Server Docs](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/advanced/historyserver/>) for details.
...
| true | Boolean | Feature toggle for enabling checkpointing even if some of tasks have finished. Before you enable it, please take a look at [the important considerations](<//nightlies.apache.org/flink/flink-docs-release-2.2//docs/dev/datastream/fault-tolerance/checkpointing/#checkpointing-with-parts-of-the-graph-finished>)
...
Please refer to the [Debugging Classloading Docs](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/debugging/debugging_classloading/>) for details.
...
Please refer to [State Backends](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/state_backends/#enabling-changelog>) for information on using State Changelog.
...
| "region" | String | This option specifies how the job computation recovers from task failures. Accepted values are:
  ...
  * 'region': Restarts all tasks that could be affected by the task failure. More details can be found [here](<//nightlies.apache.org/flink/flink-docs-release-2.2//docs/ops/state/task_failure_recovery/#restart-pipelined-region-failover-strategy>).
...
The resource requirements of a running job can be changed using the [REST API endpoint](<//nightlies.apache.org/flink/flink-docs-release-2.2//docs/ops/rest_api/#jobs-jobid-resource-requirements-1>).
...
Possible values:
  ...
  * "Adaptive": Adaptive scheduler. More details can be found [here](<//nightlies.apache.org/flink/flink-docs-release-2.2//docs/deployment/elastic_scaling#adaptive-scheduler>).
  * "AdaptiveBatch": Adaptive batch scheduler. More details can be found [here](<//nightlies.apache.org/flink/flink-docs-release-2.2//docs/deployment/elastic_scaling#adaptive-batch-scheduler>).
...
Please refer to the [network memory tuning guide](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/memory/network_mem_tuning/>) for details on how to use the `taskmanager.network.memory.buffer-debloat.*` configuration.
