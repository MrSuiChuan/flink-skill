...
要使用测试工具，还需要一组其他的依赖项，请查阅[配置](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/configuration/testing/>)小节了解更多细节。
...
关于使用 `MiniClusterWithClientResource` 进行集成测试的几点备注：
  ...
  * 如果你的作业使用事件时间计时器，则可以实现自定义的 _并行_ 源函数来发出 watermark。
