...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/data_stream_api/>).
...
Please refer to the [configuration](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/configuration/overview/>) section for more information.
...
For more information on the different execution modes, see also the corresponding [DataStream API section](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/datastream/execution_mode/>).
...
One must meet the following prerequisites before setting the runtime mode to `BATCH`:
  ...
  * Operators need a sufficient amount of [off-heap memory](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/memory/mem_setup_tm/#managed-memory>) for sorting and other intermediate results.
...
A batch execution has the following implications (among others):
  ...
  * Exchanges between tasks might be blocking according to the [`execution.batch-shuffle-mode`](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/config/#execution-batch-shuffle-mode>). This also means potentially less resource requirements compared to executing the same pipeline in streaming mode.
...
From a Table API’s perspective, converting from and to DataStream API is similar to reading from or writing to a virtual table connector that has been defined using a [`CREATE TABLE` DDL](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/sql/create/#create-table>) in SQL.
...
The virtual DataStream table source implements [`SupportsSourceWatermark`](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/sourcessinks/#source-abilities>) and thus allows calling the `SOURCE_WATERMARK()` built-in function as a watermark strategy to adopt watermarks from the DataStream API.
...
Since `DataType` is richer than `TypeInformation`, we can easily enable immutable POJOs and other complex data structures. The following example in Java shows what is possible. Check also the [Data Types & Serialization](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/datastream/fault-tolerance/serialization/types_serialization/>) page of the DataStream API for more information about the supported types there.
...
Internally, Flink’s table runtime is a changelog processor. The concepts page describes how [dynamic tables and streams relate](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/concepts/dynamic_tables/>) to each other.
...
The virtual table source implements [`SupportsSourceWatermark`](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/sourcessinks/#source-abilities>).
...
For more information about which conversions are supported for data types in Example 4, see the [Table API’s Data Types page](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/types/>).
...
**Note:** A detailed discussion about dynamic tables and their properties is given in the [Dynamic Tables](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/concepts/dynamic_tables/>) document.
...
Position-based mapping can be used to give fields a more meaningful name while keeping the field order. This mapping is available for composite data types _with a defined field order_ and atomic types. Composite data types such as tuples, rows, and case classes have such a field order. However, fields of a POJO must be mapped based on the field names (see next section). Fields can be projected out but can’t be renamed using an alias `as`(Java and Scala) or `alias`(Python).
...
Flink supports POJOs as composite types. The rules for what determines a POJO are documented [here](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/datastream/fault-tolerance/serialization/types_serialization/#pojos>).
