...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/release-notes/flink-1.13/>).
...
This feature is not covering all locations in Flink where user code is executed. It just adds the infrastructure for such an interception. We are tracking this improvement in [FLINK-21307](<https://issues.apache.org/jira/browse/FLINK-21307>).
...
The unified source API for connectors has a minor breaking change. The `SplitEnumerator.snapshotState()` method was adjusted to accept the _Checkpoint ID_ of the checkpoint for which the snapshot is created.
