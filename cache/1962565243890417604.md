...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/dev/dataset/examples/>).

#  Batch Examples #

The following example programs showcase different applications of Flink from simple word counting to graph algorithms. The code samples illustrate the use of [Flink’s DataSet API](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/dev/dataset/overview/>).

The full source code of the following and more examples can be found in the [ flink-examples-batch ](<//github.com/apache/flink/blob/release-1.15/flink-examples/flink-examples-batch>) module of the Flink source repository.
...
The [ WordCount example ](<//github.com/apache/flink/blob/release-1.15//flink-examples/flink-examples-batch/src/main/java/org/apache/flink/examples/java/wordcount/WordCount.java>) implements the above described algorithm with input parameters: `--input <path> --output <path>`. As test data, any text file will do.
...
The [ WordCount example ](<//github.com/apache/flink/blob/release-1.15//flink-examples/flink-examples-batch/src/main/scala/org/apache/flink/examples/scala/wordcount/WordCount.scala>) implements the above described algorithm with input parameters: `--input <path> --output <path>`. As test data, any text file will do.

##  Page Rank #

The PageRank algorithm computes the “importance” of pages in a graph defined by links, which point from one pages to another page. It is an iterative graph algorithm, which means that it repeatedly applies the same computation. In each iteration, each page distributes its current rank over all its neighbors, and compute its new rank as a taxed sum of the ranks it received from its neighbors. The PageRank algorithm was popularized by the Google search engine which uses the importance of webpages to rank the results of search queries.

In this simple example, PageRank is implemented with a [bulk iteration](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/dev/dataset/iterations/>) and a fixed number of iterations.
...
The [ PageRank program ](<//github.com/apache/flink/blob/release-1.15//flink-examples/flink-examples-batch/src/main/java/org/apache/flink/examples/java/graph/PageRank.java>) implements the above example. It requires the following parameters to run: `--pages <path> --links <path> --output <path> --numPages <n> --iterations <n>`.
...
The [ PageRank program ](<//github.com/apache/flink/blob/release-1.15//flink-examples/flink-examples-batch/src/main/scala/org/apache/flink/examples/scala/graph/PageRankBasic.scala>) implements the above example. It requires the following parameters to run: `--pages <path> --links <path> --output <path> --numPages <n> --iterations <n>`.
...
This implementation uses a [delta iteration](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/dev/dataset/iterations/>): Vertices that have not changed their component ID do not participate in the next step. This yields much better performance, because the later iterations typically deal only with a few outlier vertices.
...
The [ ConnectedComponents program ](<//github.com/apache/flink/blob/release-1.15//flink-examples/flink-examples-batch/src/main/java/org/apache/flink/examples/java/graph/ConnectedComponents.java>) implements the above example. It requires the following parameters to run: `--vertices <path> --edges <path> --output <path> --iterations <n>`.
...
The [ ConnectedComponents program ](<//github.com/apache/flink/blob/release-1.15//flink-examples/flink-examples-batch/src/main/scala/org/apache/flink/examples/scala/graph/ConnectedComponents.scala>) implements the above example. It requires the following parameters to run: `--vertices <path> --edges <path> --output <path> --iterations <n>`.
