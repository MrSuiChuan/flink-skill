...
Flink 的流连接器并不会放到发行文件里面一同发布，阅读[此文档](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/configuration/overview/>)，了解如何将连接器添加到集群实例内。

##  Pulsar Source #

> Pulsar Source 基于 Flink 最新的[批流一体 API](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/datastream/sources/>) 进行开发。
...
同时使用 Pulsar 的 `Schema` 以及在 builder 中指定 `PulsarSourceBuilder.enableSchemaEvolution()` 可以启用 [Schema evolution](<https://pulsar.apache.org/docs/2.11.x/schema-evolution-compatibility/#schema-evolution>) 特性。该特性会使用 Pulsar Broker 端提供的 Schema 版本兼容性检测以及 Schema 版本演进。下列示例展示了如何启用 Schema Evolution。
...
[这篇文档](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/datastream/event-time/generating_watermarks/>)详细讲解了如何定义 `WatermarkStrategy`。
...
同时使用 Pulsar 的 `Schema` 以及在 builder 中指定 `PulsarSinkBuilder.enableSchemaEvolution()` 可以启用 [Schema evolution](<https://pulsar.apache.org/docs/2.11.x/schema-evolution-compatibility/#schema-evolution>) 特性。该特性会使用 Pulsar Broker 端提供的 Schema 版本兼容性检测以及 Schema 版本演进。下列示例展示了如何启用 Schema Evolution。
...
可以通过继承 `PulsarSerializationSchema` 接口来实现自定义的序列化逻辑。接口需要返回一个类型为 `PulsarMessage` 的消息，此类型实例无法被直接创建，连接器提供了构造方法并定义了三种消息类型的构建。
  ...
  * 创建一个消息体为空的墓碑消息。[墓碑](<https://en.wikipedia.org/wiki/Tombstone_%28data_store%29>) 是一种特殊的消息，并在 Pulsar 中提供了支持。
...
[Pulsar Schema Evolution](<https://pulsar.apache.org/docs/2.11.x/schema-evolution-compatibility/#schema-evolution>) 允许用户在一个 Flink 应用程序中使用的数据模型发生特定改变后（比如向基于 ARVO 的 POJO 类中增加或删除一个字段），仍能使用同一个 Flink 应用程序的代码。

可以在 Pulsar 集群内指定哪些类型的数据模型的改变是被允许的，详情请参阅 [Pulsar Schema Evolution](<https://pulsar.apache.org/docs/2.11.x/schema-evolution-compatibility/#schema-evolution>)。
...
在 [FLIP-33: Standardize Connector Metrics](<https://cwiki.apache.org/confluence/display/FLINK/FLIP-33%3A+Standardize+Connector+Metrics>) 定义的基础 Source 指标之上，我们额外提供了一些来自 Client 的监控指标。你需要启用 `pulsar.source.enableMetrics` 选项来获得这些监控指标，所有的指标列举在下面的表格中。
...
下列表格列出了当前 Sink 支持的监控指标，前 6 个指标是 [FLIP-33: Standardize Connector Metrics](<https://cwiki.apache.org/confluence/display/FLINK/FLIP-33%3A+Standardize+Connector+Metrics>) 中规定的 Sink 连接器应当支持的标准指标。前 5 个指标会默认暴露给用户，其他指标需要通过启用 `pulsar.sink.enableMetrics` 选项来获得。
...
常见的升级步骤，请参阅[升级应用程序和 Flink 版本](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/ops/upgrading/>)。Pulsar 连接器没有在 Flink 端存储消费的状态，所有的消费信息都推送到了 Pulsar。所以需要注意下面的事项：
...
Pulsar 事务机制仍在积极发展中，当前版本并不稳定。 Pulsar 2.9.2 引入了这个问题 [a break change](<https://github.com/apache/pulsar/pull/13135>)。 如果您使用 Pulsar 2.9.2或更高版本与较旧的 Pulsar 客户端一起使用，您可能会收到一个“TransactionCoordinatorNotFound”异常。
