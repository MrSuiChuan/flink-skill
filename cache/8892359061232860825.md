...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/libs/cep/>).
...
如果你想现在开始尝试，[创建一个 Flink 程序](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/configuration/overview/>)， 添加 FlinkCEP 的依赖到项目的`pom.xml`文件中。
...
> FlinkCEP 不是二进制发布包的一部分。在集群上执行如何链接它可以看[这里](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/configuration/overview/>)。
...
也可以为模式定义一个有效时间约束。 例如，你可以通过`pattern.within()`方法指定一个模式应该在10秒内发生。 这种时间模式支持[处理时间和事件时间](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/concepts/time/>).
...
`PatternProcessFunction`可以访问`Context`对象。有了它之后，你可以访问时间属性，比如`currentProcessingTime`或者当前匹配的`timestamp` （最新分配到匹配上的事件的时间戳）。 更多信息可以看时间上下文。 通过这个上下文也可以将结果输出到[侧输出](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/datastream/side_output/>).
...
Note `processTimedOutMatch`不能访问主输出。 但你可以通过`Context`对象把结果输出到[侧输出](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/datastream/side_output/>)。
