...
> Example Consider the code written against a `Public` API in 1.15.2:
>
...
> Example Assuming a release sequence of 1.18, 1.19, 1.20, 2.0, 2.1, …, 3.0,
>
...
The line of action for upgrading a streaming application or migrating an application to a different cluster is based on Flink’s [Savepoint](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/ops/state/savepoints/>) feature. A savepoint is a consistent snapshot of the state of an application at a specific point in time.
...
See the [table state management section](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/concepts/overview/#state-management>) for more information.
...
Before starting the migration, please check that the jobs you are trying to migrate are following the best practices for [savepoints](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/ops/state/savepoints/>).
...
For more details, please read the [savepoint documentation](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/ops/state/savepoints/>).
...
If you are unfamiliar with installing Flink in your cluster, please read the [deployment and cluster setup documentation](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/resource-providers/standalone/overview/>).
...
For more details, please take a look at the [savepoint documentation](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/ops/state/savepoints/>).

##  Compatibility Table #

Savepoints are compatible across Flink versions as indicated by the table below:

Created with \ Resumed with | 1.17.x | 1.18.x | 1.19.x | 1.20.x | Limitations
---|---|---|---|---|---
**1.8.x** | O | O | O | O |
**1.9.x** | O | O | O | O |
**1.10.x** | O | O | O | O |
**1.11.x** | O | O | O | O |
**1.12.x** | O | O | O | O |
**1.13.x** | O | O | O | O | Don't upgrade from 1.12.x to 1.13.x with an unaligned checkpoint. Please use a savepoint for migrating.
**1.14.x** | O | O | O | O |
...
Please refer to the last [Compatibility Table](<https://nightlies.apache.org/flink/flink-docs-release-1.18/docs/ops/upgrading/#compatibility-table>) for the savepoint compatibility information of older Flink versions.
