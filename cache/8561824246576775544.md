...
Unlike [ORDER BY](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/table/hive-compatibility/hive-dialect/queries/overview/#order-by-clause>) which guarantees a total order of output, `SORT BY` only guarantees the result rows with each partition is in the user specified order. So when there’s more than one partition, `SORT BY` may return result that’s partially ordered.
