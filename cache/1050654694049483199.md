...
State is accessed using the `RuntimeContext`, so it is only possible in _rich functions_. Please see [here](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/datastream/user_defined_functions/#rich-functions>) for information about that, but we will also see an example shortly. The `RuntimeContext` that is available in a `RichFunction` has these methods for accessing state:
...
> Currently the ForSt State Backend only cleanup the state in compaction process. If you are using a state backend other than ForSt and want to read about other cleanup strategies, please refer to the [State V1 documentation](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/datastream/fault-tolerance/state/#cleanup-of-expired-state>) about the expired state cleanup.
...
_Operator State_ (or _non-keyed state_) is state that is bound to one parallel operator instance. The [Kafka Connector](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/connectors/datastream/kafka/>) is a good motivating example for the use of Operator State in Flink. Each parallel instance of the Kafka consumer maintains a map of topic partitions and offsets as its Operator State.
