...
A prerequisite for the commands listed in this section to work is to have a running Flink deployment like [Kubernetes](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/resource-providers/native_kubernetes/>), [YARN](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/resource-providers/yarn/>) or any other option available. Feel free to [start a Flink cluster locally](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/resource-providers/standalone/overview/#starting-a-standalone-cluster-session-mode>) to try the commands on your own machine.
...
The `run` and `run-application` commands support passing additional configuration parameters via the `-D` argument. For example setting the [maximum parallelism](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/config/#pipeline-max-parallelism#application-mode>) for a job can be done by setting `-Dpipeline.max-parallelism=120`. This argument is very useful for configuring application mode clusters, because you can pass any configuration parameter to the cluster without changing the configuration file.

When submitting a job to an existing session cluster, only [execution configuration parameters](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/config/#execution>) are supported.
...
[Savepoints](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/ops/state/savepoints/>) can be created to save the current state a job is in. All that’s needed is the JobID:
...
The savepoint folder is optional and needs to be specified if [state.savepoints.dir](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/config/#state-savepoints-dir>) isn’t set.

Lastly, you can optionally provide what should be the [binary format](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/ops/state/savepoints/#savepoint-format>) of the savepoint.
...
We have to use `--savepointPath` to specify the savepoint folder if [state.savepoints.dir](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/config/#state-savepoints-dir>) isn’t set.
...
You can also select the [restore mode](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/ops/state/savepoints/#restore-mode>) which should be used for the savepoint. The mode controls who takes ownership of the files of the specified savepoint.
...
`run-application` |  This action executes jobs in [ Application Mode](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/overview/#application-mode>). Other than that, it requires the same parameters as the `run` action.
...
`savepoint` |  This action can be used to create or disposing savepoints for a given job. It might be necessary to specify a savepoint directory besides the JobID, if the [state.savepoints.dir](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/config/#state-savepoints-dir>) parameter was not specified in `conf/flink-conf.yaml`.
...
The Flink cluster can be also managed using the [REST API](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/ops/rest_api/>). The commands described in previous sections are a subset of what is offered by Flink’s REST endpoints. Therefore, tools like `curl` can be used to get even more out of Flink.

###  Selecting Deployment Targets #

Flink is compatible with multiple cluster management frameworks like [Kubernetes](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/resource-providers/native_kubernetes/>) or [YARN](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/resource-providers/yarn/>) which are described in more detail in the Resource Provider section. Jobs can be submitted in different [Deployment Modes](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/overview/#deployment-modes>). The parameterization of a job submission differs based on the underlying framework and Deployment Mode.

`bin/flink` offers a parameter `--target` to handle the different options. In addition to that, jobs have to be submitted using either `run` (for [Session](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/overview/#session-mode>) and [Per-Job Mode (deprecated)](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/overview/#per-job-mode>)) or `run-application` (for [Application Mode](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/overview/#application-mode>)). See the following summary of parameter combinations:
...
The `--target` will overwrite the [execution.target](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/config/#execution-target>) specified in the `conf/flink-conf.yaml`.
...
The following commands show different PyFlink job submission use-cases:
  ...
  * Run a PyFlink job using a [YARN cluster in Per-Job Mode](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/resource-providers/yarn/#per-job-cluster-mode>):
  ...
  * Run a PyFlink job using a [YARN cluster in Application Mode](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/resource-providers/yarn/#application-mode>):
...
Note The archive files specified via `-pyarch` will be distributed to the TaskManagers through blob server where the file size limit is 2 GB. If the size of an archive file is more than 2 GB, you could upload it to a distributed file system and then use the path in the command line option `-pyarch`.
  ...
  * Run a PyFlink application on a native Kubernetes cluster having the cluster ID `<ClusterId>`, it requires a docker image with PyFlink installed, please refer to [Enabling PyFlink in docker](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/resource-providers/standalone/docker/#enabling-python>):
...
To learn more available options, please refer to [Kubernetes](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/resource-providers/native_kubernetes/>) or [YARN](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/resource-providers/yarn/>) which are described in more detail in the Resource Provider section.
...
In addition to the command line options during submitting the job, it also supports to specify the dependencies via configuration or Python API inside the code. Please refer to the [dependency management](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/python/dependency_management/>) for more details.
