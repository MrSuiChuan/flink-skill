...
See [MCP](<//nightlies.apache.org/flink/flink-agents-docs-release-0.3/docs/development/mcp/>) for details.
...
When the tool request comes from `chat_model_action`, the emitted `ToolResponseEvent` is automatically consumed by `chat_model_action` to continue the chat. See [Built-in Events and Actions in Chat Models](<//nightlies.apache.org/flink/flink-agents-docs-release-0.3/docs/development/chat_models/#built-in-events-and-actions>) for details on how `chat_model_action` handles tool responses.
