...
There is no connector (yet) available for Flink version 2.0.

In order to use the in PyFlink jobs, the following dependencies are required:

Version | PyFlink JAR | flink-connector-pulsar | There is no SQL jar (yet) available for Flink version 2.0.
---|---
See [Python dependency management](<//nightlies.apache.org/flink/flink-docs-release-2.0//docs/dev/python/dependency_management/#jar-dependencies>) for more details on how to use JARs in PyFlink.

Flink’s streaming connectors are not part of the binary distribution. See how to link with them for cluster execution [here](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/configuration/overview/>).

##  Pulsar Source #

> This part describes the Pulsar source based on the new [data source](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/sources/>) API.
...
[This documentation](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/event-time/generating_watermarks/>) describes details about how to define a `WatermarkStrategy`.
...
The generic upgrade steps are outlined in [upgrading jobs and Flink versions guide](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/ops/upgrading/>). The Pulsar connector does not store any state on the Flink side. The Pulsar connector pushes and stores all the states on the Pulsar side. For Pulsar, you additionally need to know these limitations:
