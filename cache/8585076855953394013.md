...
你需要为使用的扩展资源准备插件，并将其放入 Flink 发行版的 `plugins/` 文件夹中, 参看 [Flink Plugins](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/filesystems/plugins/>)。 Flink 提供了第一方的 GPU 资源插件。你同样可以为你所使用的扩展资源实现自定义插件实现自定义插件。
...
之后，将 `FPGADriver`，`FPGADriverFactory`，`META-INF/services/` 和所有外部依赖打入 jar 包。在你的 Flink 发行版的 `plugins/` 文件夹中创建一个名为“fpga”的文件夹，将打好的 jar 包放入其中。 更多细节请查看 [Flink Plugin](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/filesystems/plugins/>)。
...
我们提供了 [ 一个示例程序 ](<//github.com/apache/flink/blob/release-2.3//flink-examples/flink-examples-streaming/src/main/java/org/apache/flink/streaming/examples/gpu/MatrixVectorMul.java>) ，展示了如何在 Flink 中使用 GPU 资源来做矩阵-向量乘法。
