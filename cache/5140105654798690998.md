...
Flink supports `TUMBLE`, `HOP` and `CUMULATE` types of window aggregations. In streaming mode, the time attribute field of a window table-valued function must be on either [event or processing time attributes](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/dev/table/concepts/time_attributes/>). See [Windowing TVF](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/dev/table/sql/queries/window-tvf/>) for more windowing functions information. In batch mode, the time attribute field of a window table-valued function must be an attribute of type `TIMESTAMP` or `TIMESTAMP_LTZ`.
...
>
>   * Have all performance optimizations mentioned in [Performance Tuning](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/dev/table/tuning/>).
>   * Support standard `GROUPING SETS` syntax.
>   * Can apply [Window TopN](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/dev/table/sql/queries/window-topn/>) after window aggregation result.
>   * and so on.
>
...
| Returns the timestamp of the _exclusive_ upper bound of the corresponding tumbling, hopping, or session window. **Note:** The exclusive upper bound timestamp _cannot_ be used as a [rowtime attribute](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/dev/table/concepts/time_attributes/>) in subsequent time-based operations, such as [interval joins](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/dev/table/sql/queries/joins/#interval-joins>) and [group window](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/dev/table/sql/queries/window-agg/>) or [over window aggregations](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/dev/table/sql/queries/over-agg/>).
