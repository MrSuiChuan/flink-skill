...
This page describes deploying a [standalone](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/resource-providers/standalone/overview/>) Flink cluster on top of Kubernetes, using Flink’s standalone deployment. We generally recommend new users to deploy Flink on Kubernetes using [native Kubernetes deployments](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/resource-providers/native_kubernetes/>).
...
A _Flink Session cluster_ deployment in Kubernetes has at least three components:
  ...
  * a _Deployment_ which runs a [JobManager](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/concepts/glossary/#flink-jobmanager>)
  * a _Deployment_ for a pool of [TaskManagers](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/concepts/glossary/#flink-taskmanager>)
...
The `args` attribute in the `jobmanager-job.yaml` has to specify the main class of the user job. See also [how to specify the JobManager arguments](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/resource-providers/standalone/docker/#jobmanager-additional-command-line-arguments>) to understand how to pass other `args` to the Flink image in the `jobmanager-job.yaml`.

The _job artifacts_ should be available from the `job-artifacts-volume` in the resource definition examples. The definition examples mount the volume as a local directory of the host assuming that you create the components in a minikube cluster. If you do not use a minikube cluster, you can use any other type of volume, available in your Kubernetes cluster, to supply the _job artifacts_. Alternatively, you can build [a custom image](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/resource-providers/standalone/docker/#advanced-customization>) which already contains the artifacts instead.
...
All configuration options are listed on the [configuration page](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/config/>). Configuration options can be added to the `flink-conf.yaml` section of the `flink-configuration-configmap.yaml` config map.
...
Session Mode and Application Mode clusters support using the [Kubernetes high availability service](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/ha/kubernetes_ha/>). You need to add the following Flink config options to flink-configuration-configmap.yaml.

Note The filesystem which corresponds to the scheme of your configured HA storage directory must be available to the runtime. Refer to [custom Flink image](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/resource-providers/standalone/docker/#advanced-customization>) and [enable plugins](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/resource-providers/standalone/docker/#using-filesystem-plugins>) for more information.
