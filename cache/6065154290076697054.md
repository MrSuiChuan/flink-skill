...
Copied to clipboard!  |  [Download](<https://repo.maven.apache.org/maven2/org/apache/flink/flink-sql-connector-kinesis/5.0.0-1.19/flink-sql-connector-kinesis-5.0.0-1.19.jar>)
---|---

The Kinesis connector is not part of the binary distribution. See how to link with it for cluster execution [here](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/configuration/overview/>).
...
| optional | no | LATEST | String | Initial position to be used when reading from the table.

##### source.init.timestamp

| optional | no | (none) | String | The initial timestamp to start reading Kinesis stream from (when `source.init.position` is AT_TIMESTAMP).

##### source.init.timestamp.format

| optional | no | yyyy-MM-dd'T'HH:mm:ss.SSSXXX | String | The date format of initial timestamp to start reading Kinesis stream from (when `source.init.position` is AT_TIMESTAMP).
...
> Refer to the [Kinesis Datastream API](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/connectors/datastream/kinesis/>) documentation for more detailed description of features.
