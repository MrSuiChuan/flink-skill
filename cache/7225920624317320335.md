...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/internals/sinks/>).
...
If you are looking for pre-defined sink connectors, please check the [Connector Docs](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/connectors/datastream/overview/>).
...
The [ Sink ](<//github.com/apache/flink/blob/master/flink-core/src/main/java/org/apache/flink/api/connector/sink2/Sink.java>) API is a factory style interface to create the SinkWriter to write the data.
...
The core [ SinkWriter ](<//github.com/apache/flink/blob/master/flink-core/src/main/java/org/apache/flink/api/connector/sink2/SinkWriter.java>) API is responsible for writing data to downstream system.

The `SinkWriter` API only has three methods:

  * write(InputT element, Context context): Adds an element to the writer.
  * flush(boolean endOfInput): Called on checkpoint or end of the input, setting this flag causes the writer to flush all pending data for `at-least-once`. To archive `exactly-once` semantic, the writer should implement the SupportsCommitter interface.
...
The [ SupportsWriterState ](<//github.com/apache/flink/blob/master/flink-core/src/main/java/org/apache/flink/api/connector/sink2/SupportsWriterState.java>) interface is used to indicate that the sink supports writer state, which means that the sink can be recovered from a failure.

The `SupportsWriterState` interface requires the `SinkWriter` to implement the [ StatefulSinkWriter ](<//github.com/apache/flink/blob/master/flink-core/src/main/java/org/apache/flink/api/connector/sink2/StatefulSinkWriter.java>) interface.

###  SupportsCommitter #

The [ SupportsCommitter ](<//github.com/apache/flink/blob/master/flink-core/src/main/java/org/apache/flink/api/connector/sink2/SupportsCommitter.java>) interface is used to indicate that the sink supports exactly-once semantics using a two-phase commit protocol.
...
The following figure shows the operator topology of using [ SupportsPreWriteTopology ](<//github.com/apache/flink/blob/master/flink-runtime/src/main/java/org/apache/flink/streaming/api/connector/sink2/SupportsPreWriteTopology.java>) :
...
The following figure shows the operator topology of using [ SupportsPreCommitTopology ](<//github.com/apache/flink/blob/master/flink-runtime/src/main/java/org/apache/flink/streaming/api/connector/sink2/SupportsPreCommitTopology.java>) :
...
The following figure shows the operator topology of using [ SupportsPostCommitTopology ](<//github.com/apache/flink/blob/master/flink-runtime/src/main/java/org/apache/flink/streaming/api/connector/sink2/SupportsPostCommitTopology.java>) :
