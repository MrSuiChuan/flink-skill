...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/python/python_config/>).
...
For Python Table API program, all the config options available for Java/Scala Table API program could also be used in the Python Table API program. You could refer to the [Table API Configuration](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/config/>) for more details on all the available config options for Table API programs. The config options could be set as following in a Table API program:
...
| (none) | String | Specify the path on the Worker Node where the Flink Python Dependencies are installed, which gets added into the PYTHONPATH of the Python Worker. The option is equivalent to the command line option "-pypath".
...
| true | Boolean | Specify whether to load System Environment when starting Python worker.