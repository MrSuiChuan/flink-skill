...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/table/materialized-table/statements/>).
...
`WITH Options` 可以定义创建物化表所需的属性，包括[连接器参数](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/connectors/table/>)和分区字段的[时间格式参数](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/config/#partition-fields-date-formatter>)。
...
注意
  ...
  * [partition.fields.#.date-formatter](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/config/#partition-fields-date-formatter>) 选项仅适用于全量模式。
  * [partition.fields.#.date-formatter](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/config/#partition-fields-date-formatter>) 中的字段必须是有效的字符串类型分区字段。
...
数据新鲜度定义了物化表内容滞后于基础表更新的最长时间。它有两个作用，首先通过[配置](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/config/#materialized-table-refresh-mode-freshness-threshold>)确定物化表的[刷新模式](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/materialized-table/overview/#%e5%88%b7%e6%96%b0%e6%a8%a1%e5%bc%8f>)，然后确定数据刷新频率以满足实际数据新鲜度要求。
...
注意
  ...
  * 在连续模式下，数据新鲜度和 `checkpoint` 间隔一致，设置过短的数据新鲜度可能会对作业性能产生影响。此外，为了优化 `checkpoint` 性能，建议[开启 Changelog](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/state_backends/#%e5%bc%80%e5%90%af-changelog>)。
...
该子句用于定义填充物化表数据的查询。上游表可以是物化表、表或视图。select 语句支持所有 Flink SQL [查询](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/overview/>)。
...
注意
  ...
  * 暂停连续模式的表时，默认会使用 `STOP WITH SAVEPOINT` 暂停作业，你需要使用[参数](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#execution-checkpointing-savepoint-dir>)设置 `SAVEPOINT` 保存路径。
