...
本文描述了如何使用 Flink standalone 部署模式在 Kubernetes 上部署 [standalone](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/resource-providers/standalone/overview/>) 模式的 Flink 集群。通常我们建议新用户使用 [native Kubernetes 部署](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/resource-providers/native_kubernetes/>)模式在 Kubernetes上部署 Flink。
...
在 Kubernetes 上部署一个基本的 _Flink session 集群_ 时，一般包括下面三个组件：
  ...
  * 运行 [JobManager](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/concepts/glossary/#flink-jobmanager>) 的 _Deployment_ ；
  * 运行 [TaskManagers](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/concepts/glossary/#flink-taskmanager>) 的 _Deployment_ ；
...
`jobmanager-application-non-ha.yaml` 中的 `args` 属性必须指定用户作业的主类。也可以参考[如何设置 JobManager 参数](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/resource-providers/standalone/docker/#jobmanager-additional-command-line-arguments>)来了解如何将额外的 `args` 传递给 `jobmanager-application-non-ha.yaml` 配置中指定的 Flink 镜像。
...
_job artifacts_ 可由以下方式提供：
  ...
  * 构建一个已经包含 _job artifacts_ 参数的[自定义镜像](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/resource-providers/standalone/docker/#advanced-customization>)。
  * 通过指定[–jars](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/resource-providers/standalone/docker/#jobmanager-additional-command-line-arguments>)参数提供 存储在DFS或者可由HTTP/HTTPS下载的 _job artifacts_ 路径
...
所有配置项都展示在[配置页面](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/config/>)上。在 config map 配置文件 `flink-configuration-configmap.yaml` 中，可以将配置添加在 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/config/#flink-配置文件>) 部分。
...
对于在 Kubernetes 上实现HA，可以参考当前的 [Kubernets 高可用服务](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/ha/overview/>)。

####  Kubernetes 高可用 Services #

Session 模式和 Application 模式集群都支持使用 [Kubernetes 高可用服务](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/ha/kubernetes_ha/>)。需要在 flink-configuration-configmap.yaml 中添加如下 Flink 配置项。

Note 配置了 HA 存储目录相对应的文件系统必须在运行时可用。请参阅[自定义Flink 镜像](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/resource-providers/standalone/docker/#advanced-customization>)和[启用文件系统插件](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/resource-providers/standalone/docker/#using-filesystem-plugins>)获取更多相关信息。
