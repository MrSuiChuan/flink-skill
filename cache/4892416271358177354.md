...
This page describes deploying a [standalone](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/resource-providers/standalone/overview/>) Flink cluster on top of Kubernetes, using Flink’s standalone deployment. We generally recommend new users to deploy Flink on Kubernetes using [native Kubernetes deployments](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/resource-providers/native_kubernetes/>).
...
A _Flink Session cluster_ deployment in Kubernetes has at least three components:
  ...
  * a _Deployment_ which runs a [JobManager](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/concepts/glossary/#flink-jobmanager>)
  * a _Deployment_ for a pool of [TaskManagers](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/concepts/glossary/#flink-taskmanager>)
...
The _job artifacts_ could be provided by these way：
  ...
  * You can build [a custom image](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/resource-providers/standalone/docker/#advanced-customization>) which already contains the artifacts instead.
  * You can pass artifacts via the [–jars](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/resource-providers/standalone/docker/#jobmanager-additional-command-line-arguments>) option that are stored locally, on [remote DFS](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/filesystems/overview/>), or accessible via an HTTP(S) endpoint.
...
All configuration options are listed on the [configuration page](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/>). Configuration options can be added to the [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file>) section of the `flink-configuration-configmap.yaml` config map.
...
Session Mode and Application Mode clusters support using the [Kubernetes high availability service](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/ha/kubernetes_ha/>). You need to add the following Flink config options to flink-configuration-configmap.yaml.

Note The filesystem which corresponds to the scheme of your configured HA storage directory must be available to the runtime. Refer to [custom Flink image](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/resource-providers/standalone/docker/#advanced-customization>) and [enable plugins](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/resource-providers/standalone/docker/#using-filesystem-plugins>) for more information.
    ...
    spec:
      ...
      template:
        ...
        spec:
          containers:
          - name: jobmanager
            image: apache/flink:2.2.1-scala_2.12
            args: ["jobmanager"]
            ports:
            - containerPort: 6123
              name: rpc
