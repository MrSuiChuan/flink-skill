...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/ops/state/large_state_tuning/>).
...
The easiest way to monitor checkpoint behavior is via the UI’s checkpoint section. The documentation for [checkpoint monitoring](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/monitoring/checkpoint_monitoring/>) shows how to access the available checkpoint metrics.

The two numbers (both exposed via Task level [metrics](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/metrics/#checkpointing>) and in the [web interface](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/monitoring/checkpoint_monitoring/>)) that are of particular interest when scaling up checkpoints are:
...
The state storage workhorse of many large scale Flink streaming applications is the _RocksDB State Backend_. The backend scales well beyond main memory and reliably stores large [keyed state](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/datastream/fault-tolerance/state/>).

If you are handling very large state, even exceeding the local disk space of the TaskManagers, you may want to consider using the disaggregated state store [ForStStateBackend](<//nightlies.apache.org/flink/flink-docs-master/docs/deployment/config/#forst-state-backend>). This backend stores the state in a separate storage system, such as HDFS or S3, and only keeps the state metadata and cache in the TaskManagers. And the [State API V2](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/datastream/fault-tolerance/state_v2/>) is also recommended to cooperate with ForStStateBackend for large state applications.
...
See [Incremental Checkpoints in RocksDB](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/state_backends/#incremental-checkpoints>) for more background information.
...
See [this section](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/state_backends/#timers-heap-vs-rocksdb>) for details on how to configure heap-based timers.
...
By default, the RocksDB State Backend uses Flink’s managed memory budget for RocksDBs buffers and caches (`state.backend.rocksdb.memory.managed: true`). Please refer to the [RocksDB Memory Management](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/state_backends/#memory-management>) for background on how that mechanism works.
...
This section discusses how to decide how many resources should be used for a Flink job to run reliably. The basic rules of thumb for capacity planning are:
  ...
  * Normal operation should have enough capacity to not operate under constant _back pressure_. See [back pressure monitoring](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/monitoring/back_pressure/>) for details on how to check whether the application runs under back pressure.
...
Note that [unaligned checkpoints](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/checkpoints/#unaligned-checkpoints>) currently do not support task-local recovery.
