...
`JdbcCatalog` 使得用户可以将 Flink 通过 JDBC 协议连接到关系数据库。Postgres Catalog 和 MySQL Catalog 是目前 JDBC Catalog 仅有的两种实现。 参考 [JdbcCatalog 文档](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/connectors/table/jdbc/>) 获取关于配置 JDBC catalog 的详细信息。

###  HiveCatalog #

`HiveCatalog` 有两个用途：作为原生 Flink 元数据的持久化存储，以及作为读写现有 Hive 元数据的接口。 Flink 的 [Hive 文档](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/connectors/table/hive/overview/>) 提供了有关设置 `HiveCatalog` 以及访问现有 Hive 元数据的详细信息。
...
从 1.18 开始， Flink 框架开始支持[时间旅行](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/sql/reference/queries/time-travel/>)查询表的历史数据。如果要查询表的历史数据，需要这张表所属于的 `catalog` 实现 `getTable(ObjectPath tablePath, long timestamp)` 方法，如下所示:
...
更多详细信息，请参考[Flink SQL CREATE DDL](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/sql/reference/ddl/create/>)。
...
注意：这里只列出了编程方式的 Catalog API，用户可以使用 SQL DDL 实现许多相同的功能。 关于 DDL 的详细信息请参考 [SQL CREATE DDL](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/sql/reference/ddl/create/>)。
...
For sql-gateway, you can add the option `table.catalog-modification.listeners` in the [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/config/#flink-配置文件>) and start the gateway, or you can also start sql-gateway with dynamic parameter, then you can use sql-client to perform ddl directly.
...
在 SQL Gateway 中，推荐在 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/config/#flink-配置文件>)中进行配置，所有的 session 可以自动使用已经创建好的 Catalog 。 配置的格式如下，一般情况下需要配置 Catalog Store 的 kind ，以及 Catalog Store 需要的其他参数配置。
