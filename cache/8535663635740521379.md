...
The second query is similar to the first one but groups the `clicks` table in addition to the `user` attribute also on an [hourly tumbling window](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/table/sql/queries/window-agg/#group-window-functions>) before it counts the number of URLs (time-based computations such as windows are based on special [time attributes](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/table/concepts/time_attributes/>) are discussed later). Again, the figure shows the input and output at different points in time to visualize the changing nature of dynamic tables.
...
The [Query Configuration](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/table/config/>) page discusses parameters to control the execution of continuous queries. Some parameters can be used to trade the size of the maintained state for result accuracy.
...
The API to convert a dynamic table into a `DataStream` is discussed on the [Common Concepts](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/table/common/#convert-a-table-into-a-datastream>) page. Please note that only append and retract streams are supported when converting a dynamic table into a `DataStream`. The `TableSink` interface to emit a dynamic table to an external system are discussed on the [TableSources and TableSinks](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/table/sourcessinks/#dynamic-table-sink>) page.
