...
See [MCP](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/development/mcp/>) for details.
...
`{variable_name}` is the only template syntax. When a prompt is rendered:
  ...
  * There is **no** `{{` / `}}` escape. Unlike Python f-strings and `str.format`, doubled braces are _not_ collapsed to single braces — they reach the LLM as literal `{{` and `}}`.
