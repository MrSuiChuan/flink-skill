...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/ops/monitoring/checkpoint_monitoring/>).
...
Note that for failed checkpoints, metrics are updated on a best efforts basis and may be not accurate.
  ...
  * **Status** : The current status of the checkpoint, which is either _In Progress_ , _Completed_ , or _Failed_. If the triggered checkpoint is a savepoint, you will see a floppy-disk symbol.
