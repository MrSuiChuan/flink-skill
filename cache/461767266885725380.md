...
[SQL Hints](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/dev/table/sql/queries/hints/>) can be used to apply configurations to a Hive table without changing its definition in the Hive metastore.
...
You can use a Hive table as a temporal table, and then a stream can correlate the Hive table by temporal join. Please see [temporal join](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/dev/table/sql/queries/joins/#temporal-joins>) for more information about the temporal join.
...
Please see the [streaming sink](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/connectors/table/filesystem/#streaming-sink>) for a full list of available configurations.
...
**NOTE:**
  ...
  * Currently, `DISTRIBUTED BY` and `SORTED BY` is only supported when using [Hive dialect](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/dev/table/hive-compatibility/hive-dialect/overview/>) in Flink `BATCH` mode.
