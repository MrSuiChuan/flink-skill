...
CREATE statements are used to register a table/view/function into current or specified [Catalog](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/catalogs/>). A registered table/view/function can be used in SQL queries.
...
CREATE statements can be executed in [SQL CLI](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/sqlclient/>).
...
The planner will transform computed columns into a regular projection after the source. For optimization or [watermark strategy push down](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/sourcessinks/>), the evaluation might be spread across operators, performed multiple times, or skipped if not needed for the given query.
...
Computed columns are commonly used in Flink for defining [time attributes](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/concepts/time_attributes/>) in `CREATE TABLE` statements.

  * A [processing time attribute](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/concepts/time_attributes/#processing-time>) can be defined easily via `proc AS PROCTIME()` using the system’s `PROCTIME()` function.
  * An [event time attribute](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/concepts/time_attributes/#event-time>) timestamp can be pre-processed before the `WATERMARK` declaration. For example, the computed column can be used if the original field is not `TIMESTAMP(3)` type or is nested in a JSON string.
...
The key and value of expression `key1=val1` should both be string literal. See details in [Connect to External Systems](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/connectors/table/overview/>) for all the supported table properties of different connectors.
...
There are two parts in CTAS, the SELECT part can be any [SELECT query](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/sql/queries/overview/>) supported by Flink SQL. The CREATE part takes the resulting schema from the SELECT part and creates the target table. Similar to `CREATE TABLE`, CTAS requires the required options of the target table must be specified in WITH clause.
...
If you want to enable atomicity for CTAS, then you should make sure:
  ...
  * The sink has implemented the atomicity semantics for CTAS. You may refer to the doc for the corresponding connector sink to know the atomicity semantics is available or not. For devs who want to implement the atomicity semantics, please refer to the doc [SupportsStaging](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/sourcessinks/#sink-abilities>).
  * Set option [table.rtas-ctas.atomicity-enabled](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/config/#table-rtas-ctas-atomicity-enabled>) to `true`.
...
There are two parts in RTAS: the SELECT part can be any [SELECT query](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/sql/queries/overview/>) supported by Flink SQL, the `REPLACE TABLE` part takes the resulting schema from the `SELECT` part and replace the target table. Similar to `CREATE TABLE` and `CTAS`, RTAS requires the required options of the target table must be specified in WITH clause.
...
Check out more details at [Catalogs](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/catalogs/>).
...
If the language tag is JAVA/SCALA, the identifier is the full classpath of the UDF. For the implementation of Java/Scala UDF, please refer to [User-defined Functions](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/functions/udfs/>) for more details.

If the language tag is PYTHON, the identifier is the fully qualified name of the UDF, e.g. `pyflink.table.tests.test_udf.add`. For the implementation of Python UDF, please refer to [Python UDFs](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/python/table/udfs/python_udfs/>) for more details.

If the language tag is PYTHON, however the current program is written in Java/Scala or pure SQL, then you need to [configure the Python dependencies](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/python/dependency_management/#python-dependency-in-javascala-program>).
