...
Skills are a good fit when a capability is best described as a procedure (a runbook the agent follows) rather than a single function call. For a single, well-typed operation, prefer a [tool](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/development/tool_use/>) instead.
...
`description` | Yes | 1–1024 characters. Loaded at discovery time — write it so the agent can decide _when_ to use the skill. State both what it does and when to use it.
...
**Key points:**
  ...
  * Declaring a skill source only makes the skills _available_. A skill is exposed to a chat model only when that model lists it in its `skills` (see Enable Skills on a Chat Model).
...
**Key points:**
  ...
  * Make sure the system prompt instructs the agent to load the relevant skill before acting, for example: _“You must load the skill first and strictly follow its instructions.”_ Without this nudge, smaller models may answer directly instead of consulting the skill.

Skills work with both the [Workflow Agent](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/development/workflow_agent/>) (configure the chat model via `@chat_model_setup`/`@ChatModelSetup` as above) and the [ReAct Agent](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/development/react_agent/>) (set `skills` and `allowed_commands` on the `ReActAgent`’s chat model descriptor, and register the `Skills` resource on the execution environment with `add_resource(..., ResourceType.SKILLS, ...)`).
