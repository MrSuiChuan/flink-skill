...
Flink offers a [ Wrapper ](<//github.com/apache/flink/blob/release-2.2/flink-metrics/flink-metrics-dropwizard/src/main/java/org/apache/flink/dropwizard/metrics/DropwizardMeterWrapper.java>) that allows usage of Codahale/DropWizard meters. To use this wrapper add the following dependency in your `pom.xml`:
...
You can configure which delimiter to use for the identifier (default: `.`) by setting the `metrics.scope.delimiter` key in [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file>).
...
Which context information should be included can be configured by setting the following keys in [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#flink-configuration-file>). Each of these keys expect a format string that may contain constants (e.g. “taskmanager”) and variables (e.g. “<task_id>”) which will be replaced at runtime.
...
You can define custom variables that will be assigned to all metrics reported by a given operator using `Transformation.addMetricVariable`. For example:
...
Will assign `table_name` variable with respective values `Foo` and `Bar` to all metrics reported by the `KafkaSource`, like `numRecordsOut` or `currentOutputWatermark`. If supported by your chosen metric reporter, those additional variables will be then converted to labels or tags.

##  Reporter #

For information on how to set up Flink’s metric reporters please take a look at the [metric reporters documentation](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/metric_reporters/>).
...
Scope | Infix | Metrics | Description | Type
---|---|---|---|---
**Job-/TaskManager** | Status.FileDescriptor.Max | Count | The max number of file descriptors. | Gauge
**Job-/TaskManager** | Status.FileDescriptor.Open | Count | The total open of file descriptors. | Gauge
...
**(only available if` taskmanager.network.detailed-metrics` config option is set)** | totalQueueLen | Total number of queued buffers in all input/output channels. | Gauge
...
The metrics in this table are available for each of the following job states: INITIALIZING, CREATED, RUNNING, RESTARTING, CANCELLING, FAILING. Whether these metrics are reported depends on the [metrics.job.status.enable](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#metrics-job-status-enable>) setting.
...
> Experimental
>
> While the job is in the RUNNING state the metrics in this table provide additional details on what the job is currently doing. Whether these metrics are reported depends on the [metrics.job.status.enable](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#metrics-job-status-enable>) setting.
>
> Scope | Metrics | Description | Type
> ---|---|---|---
...
numRestarts | The total number of restarts since this job was submitted, including full restarts, fine-grained restarts and restarts triggered by rescaling. | Gauge
numRescales | The total number of restarts triggered by rescaling, including scale up and scale down. | Gauge
...
lastCheckpointCompletedTimestamp | The timestamp of the last completed checkpoint (in milliseconds). | Gauge
lastCheckpointFullSize | The full size of the last checkpoint (in bytes). | Gauge
lastCheckpointMetadataSize | The metadata file size of the last checkpoint (in bytes). | Gauge
...
**Task** | checkpointAlignmentTime | The time in nanoseconds that the last barrier alignment took to complete, or how long the current alignment has taken so far (in nanoseconds). This is the time between receiving first and the last checkpoint barrier. You can find more information in the [Monitoring State and Checkpoints section](//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/state/large_state_tuning/#monitoring-state-and-checkpoints) | Gauge
...
Scope | Metrics | Description | Type
---|---|---|---
**Task/Operator** | valueStateGetKeySize | The key size of get operation for value state | Histogram
valueStateGetValueSize | The value size of get operation for value state | Histogram
valueStateUpdateKeySize | The key size of update operation for value state | Histogram
valueStateUpdateValueSize | The value size of update operation for value state | Histogram
reducingStateGetKeySize | The key size of get operation for reducing state | Histogram
reducingStateGetValueSize | The value size of get operation for reducing state | Histogram
reducingStateAddKeySize | The key size of add operation for reducing state | Histogram
reducingStateAddValueSize | The value size of add operation for reducing state | Histogram
aggregatingStateGetKeySize | The key size of get operation for aggregating state | Histogram
aggregatingStateAddKeySize | The key size of add operation for aggregating state | Histogram
listStateGetKeySize | The key size of get operation for list state | Histogram
listStateGetValueSize | The value size of get operation for list state | Histogram
listStateAddKeySize | The key size of add operation for list state | Histogram
listStateAddValueSize | The value size of add operation for list state | Histogram
listStateAddAllKeySize | The key size of addAll operation for list state | Histogram
listStateAddAllValueSize | The value size of addAll operation for list state | Histogram
listStateUpdateKeySize | The key size of update operation for list state | Histogram
listStateUpdateValueSize | The value size of update operation for list state | Histogram
mapStateGetKeySize | The key size of get operation for map state | Histogram
mapStateGetValueSize | The value size of get operation for map state | Histogram
mapStatePutKeySize | The key size of put operation for map state | Histogram
mapStatePutValueSize | The value size of put operation for map state | Histogram
mapStateIteratorKeySize | The key size of iterator#next operation for map state | Histogram
mapStateIteratorValueSize | The value size of iterator#next operation for map state | Histogram
mapStateRemoveKeySize | The key size of remove operation for map state | Histogram
mapStateContainsKeySize | The key size of contains operation for map state | Histogram
mapStateIsEmptyKeySize | The key size of isEmpty operation for map state | Histogram

###  RocksDB #

Certain RocksDB native metrics are available but disabled by default, you can find full documentation [here](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#rocksdb-native-metrics>)

###  ForSt #

Certain ForSt native metrics are available but disabled by default, you can find full documentation [here](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#forst-native-metrics>)

Besides that, we support the following metrics:

Scope | Infix | Metrics | Description | Type
---|---|---|---|---
**Task/Operator** | forst.fileCache | hit | The hit count of ForSt state backend cache. | Counter
miss | The miss count of ForSt state backend cache. | Counter
usedBytes | The bytes cached in ForSt state backend cache. | Gauge
remainingBytes | The remaining space in the volume for the configured cache. Only available when 'state.backend.forst.cache.reserve-size' is set above 0.  | Gauge
lru.evict | The number of cache files that are evicted from LRU. | Gauge
lru.loadback | The number of cache files that are loaded back from remote storage into the LRU.  | Gauge
...
Scope | Metrics | Description | Type
---|---|---|---
**Job (only available on TaskManager)** | [<source_id>.[<source_subtask_index>.]]<operator_id>.<operator_subtask_index>.latency | The latency distributions from a given source (subtask) to an operator subtask (in milliseconds), depending on the [latency granularity](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#metrics-latency-granularity>). | Histogram
**Task** | numBytesInLocal | Attention: deprecated, use [Default shuffle service metrics](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/metrics/#default-shuffle-service>). | Counter
numBytesInLocalPerSecond | Attention: deprecated, use [Default shuffle service metrics](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/metrics/#default-shuffle-service>). | Meter
numBytesInRemote | Attention: deprecated, use [Default shuffle service metrics](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/metrics/#default-shuffle-service>). | Counter
numBytesInRemotePerSecond | Attention: deprecated, use [Default shuffle service metrics](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/metrics/#default-shuffle-service>). | Meter
numBuffersInLocal | Attention: deprecated, use [Default shuffle service metrics](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/metrics/#default-shuffle-service>). | Counter
numBuffersInLocalPerSecond | Attention: deprecated, use [Default shuffle service metrics](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/metrics/#default-shuffle-service>). | Meter
numBuffersInRemote | Attention: deprecated, use [Default shuffle service metrics](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/metrics/#default-shuffle-service>). | Counter
numBuffersInRemotePerSecond | Attention: deprecated, use [Default shuffle service metrics](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/metrics/#default-shuffle-service>). | Meter
...
watermarkAlignmentDrift |  The current drift from the minimal watermark emitted by all sources/tasks/splits that belong to the same watermark group. **Note:** Available only when watermark alignment is enabled and the first common watermark is announced. You can configure the update interval in the WatermarkStrategy. | Gauge
numSplitsProcessed | The total number of InputSplits this data source has processed (if the operator is a data source). | Gauge
**Split** | watermark.currentWatermark |  The last watermark this split has received (in milliseconds).  | Gauge
watermark.activeTimeMsPerSecond |  The time (in milliseconds) this split has been active (i.e. not paused due to watermark alignment or idle due to idleness detection) per second.  | Gauge
watermark.pausedTimeMsPerSecond |  The time (in milliseconds) this split has been paused due to watermark alignment per second.  | Gauge
watermark.idleTimeMsPerSecond |  The time (in milliseconds) this split has been marked idle by idleness detection per second.  | Gauge
watermark.accumulatedActiveTimeMs |  Accumulated time (in milliseconds) this split was active since it was registered  | Gauge
watermark.accumulatedPausedTimeMs |  Accumulated time (in milliseconds) this split was paused since it was registered  | Gauge
watermark.accumulatedIdleTimeMs |  Accumulated time (in milliseconds) this split was idle since it was registered  | Gauge

###  Connectors #

####  Kafka Connectors #

Please refer to [Kafka monitoring](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/connectors/datastream/kafka//#monitoring>).
...
Scope | Infix | Metrics | Description | Type
---|---|---|---|---
**Operator** | asyncStateProcessing | numInFlightRecords | The number of in-flight records in the async execution controller's buffers. | Gauge
activeBufferSize | The number of records which are pending to be processed. | Gauge
blockingBufferSize | The number of records which are blocked by the ongoing records. | Gauge
numBlockingKeys | The number of different keys are blocked in async execution controller. | Gauge

##  End-to-End latency tracking #

Flink allows to track the latency of records travelling through the system. This feature is disabled by default. To enable the latency tracking you must set the `latencyTrackingInterval` to a positive number in either the [Flink configuration](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#metrics-latency-interval>) or `ExecutionConfig`.
...
The `LatencyMarker`s are used to derive a distribution of the latency between the sources of the topology and each downstream operator. These distributions are reported as histogram metrics. The granularity of these distributions can be controlled in the [Flink configuration](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#metrics-latency-interval>). For the highest granularity `subtask` Flink will derive the latency distribution between every source subtask and every downstream subtask, which results in quadratic (in the terms of the parallelism) number of histograms.
...
Flink also allows to track the keyed state access latency for standard Flink state-backends or customized state backends which extending from `AbstractStateBackend`. This feature is disabled by default. To enable this feature you must set the `state.latency-track.keyed-state-enabled` to true in the [Flink configuration](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#state-backends-latency-tracking-options>).
...
Flink also allows to track the keyed state key/value size for standard Flink state-backends or customized state backends which extending from `AbstractStateBackend`. This feature is disabled by default. To enable this feature you must set the `state.size-track.keyed-state-enabled` to true in the [Flink configuration](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/deployment/config/#state-backends-size-tracking-options>).

Once tracking keyed state key/value size is enabled, Flink will sample the state size every `N` access, in which `N` is defined by `state.size-track.sample-interval`. This configuration has a default value of 100. A smaller value will get more accurate results but have a higher performance impact since it is sampled more frequently.

As the type of this key/value size metrics is histogram, `state.size-track.history-size` will control the maximum number of recorded values in history, which has the default value of 128. A larger value of this configuration will require more memory, but will provide a more accurate result.

Warning Enabling state-size metrics may impact the performance. It is recommended to only use them for debugging purposes. If state.ttl is enabled, the size of the value will include the size of the TTL-related timestamp. The value size of AggregatingState is not accounted for because AggregatingState returns a result processed by a user-defined AggregateFunction, whereas currently, only the actual stored data size in the state can be tracked.

##  REST API integration #

Metrics can be queried through the [Monitoring REST API](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/ops/rest_api/>).
