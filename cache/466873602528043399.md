...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/sql/interfaces/sql-client/>).
...
The SQL Client scripts are also located in the binary directory of Flink. Users have two options for starting the SQL Client CLI, either by starting an embedded standalone process or by connecting to a remote [SQL Gateway](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/interfaces/sql-gateway/overview/>). SQL Client default mode is `embedded`.
...
By default, the SQL Client will use the truststore configured using the `security.ssl.rest.truststore` and `security.ssl.rest.truststore-password` properties in the [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-master/docs/deployment/config/#flink-configuration-file>) on the SQL client side. If these properties aren’t explicitly configured, the client will use the default certificate stores provided by the JDK.

Note SQL Client only supports connecting to the [REST Endpoint](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/interfaces/sql-gateway/rest/#rest-api>) since version v2.
...
See also the general [SQL](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/reference/overview/>) documentation.
...
You can configure the SQL Client by setting the options below, or any valid [Flink configuration](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/config/>) entry:
...
The **changelog mode** does not materialize results and visualizes the result stream that is produced by a [continuous query](<//nightlies.apache.org/flink/flink-docs-master/docs/concepts/sql-table-concepts/dynamic_tables/#continuous-queries>) consisting of insertions (`+`) and retractions (`-`).
...
The full list of offered SQL JARs can be found on the [connection to external systems page](<//nightlies.apache.org/flink/flink-docs-master/docs/connectors/table/overview/>).

You can refer to the [configuration](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/configuration/connector/>) section for information on how to configure connector and format dependencies.
...
Attention The SQL Client does not track the status of the running Flink job after submission. The CLI process can be shutdown after the submission without affecting the detached query. Flink’s `restart strategy` takes care of the fault-tolerance. Please use the job statements to [monitor the detached query status](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/interfaces/sql-client/#monitoring-job-status>) or [stop the detached query](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/interfaces/sql-client/#terminating-a-job>).
...
For more details about creating and managing savepoints, please refer to [Job Lifecycle Management](<//nightlies.apache.org/flink/flink-docs-master/docs/deployment/cli/#job-lifecycle-management>).
...
The savepoint path could be specified with [execution.checkpointing.savepoint-dir](<//nightlies.apache.org/flink/flink-docs-master/docs/deployment/config/#state-savepoints-dir>) either in the cluster configuration or session configuration (the latter would take precedence).

For more details about stopping jobs, please refer to [Job Statements](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/reference/utility/job/#stop-job>).
