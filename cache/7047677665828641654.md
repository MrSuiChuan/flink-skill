...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/connectors/datastream/elasticsearch/>).
...
---|---
6.x |  Only available for stable versions.
7.x |  Only available for stable versions.
8.x |  Only available for stable versions.

In order to use the in PyFlink jobs, the following dependencies are required:

Version | PyFlink JAR | flink-connector-elasticsearch6 | Only available for stable releases.
---|---
flink-connector-elasticsearch7 | Only available for stable releases.
...
For the execution of your Flink program, it is recommended to build a so-called uber-jar (executable jar) containing all your dependencies (see [here](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/configuration/>) for further information).
