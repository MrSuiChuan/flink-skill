...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/dev/python/datastream/operators/overview/>).
...
DataStream programs in Flink are regular programs that implement transformations on data streams (e.g., mapping, filtering, reducing). Please see [operators](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/datastream/operators/overview/>) for an overview of the available transformations in Python DataStream API.
...
Operator chaining could be disabled in one of the following ways:
  ...
  * Disable chaining with following operators by adding a [`key_by`](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/datastream/operators/overview/#keyby>) operation, [`shuffle`](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/datastream/operators/overview/#random-partitioning>) operation, [`rescale`](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/datastream/operators/overview/#rescaling>) operation, [`rebalance`](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/datastream/operators/overview/#rescaling>) operation or [`partition_custom`](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/datastream/operators/overview/#custom-partitioning>) operation after the current operator.
  * Disable chaining with preceding operators by applying a [`start_new_chain`](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/datastream/operators/overview/#start-new-chain>) operation for the current operator.
  * Disable chaining with preceding and following operators by applying a [`disable_chaining`](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/datastream/operators/overview/#disable-chaining>) operation for the current operator.
  * Disable chaining of two operators by setting different parallelisms or different [slot sharing group](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/datastream/operators/overview/#set-slot-sharing-group>) for them.
  * You could also disable all the operator chaining via configuration [`python.operator-chaining.enabled`](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/python/python_config/#python-operator-chaining-enabled>).

##  Bundling Python Functions #

To run Python functions in any non-local mode, it is strongly recommended bundling your Python functions definitions using the config option [`python-files`](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/python/python_config/#python-files>), if your Python functions live outside the file where the `main()` function is defined. Otherwise, you may run into `ModuleNotFoundError: No module named 'my_function'` if you define Python functions in a file called `my_function.py`.
