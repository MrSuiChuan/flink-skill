...
Flink needs to be aware of the state in order to make it fault tolerant using [checkpoints](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/dev/datastream/fault-tolerance/checkpointing/>) and [savepoints](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/ops/state/savepoints/>).
...
[Queryable state](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/dev/datastream/fault-tolerance/queryable_state/>) allows you to access state from outside of Flink during runtime.

When working with state, it might also be useful to read about [Flink’s state backends](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/ops/state/state_backends/>). Flink provides different state backends that specify how and where state is stored.
...
See [Restart Strategies](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/ops/state/task_failure_recovery/#restart-strategies>) for more information.
...
Unaligned checkpointing ensures that barriers are arriving at the sink as fast as possible. It’s especially suited for applications with at least one slow moving data path, where alignment times can reach hours. However, since it’s adding additional I/O pressure, it doesn’t help when the I/O to the state backends is the bottleneck. See the more in-depth discussion in [ops](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/ops/state/checkpoints/#unaligned-checkpoints>) for other limitations.
...
[Savepoints](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/ops/state/savepoints/>) are **manually triggered checkpoints** , which take a snapshot of the program and write it out to a state backend. They rely on the regular checkpointing mechanism for this.
...
Flink executes [batch programs](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/dev/dataset/overview/>) as a special case of streaming programs, where the streams are bounded (finite number of elements). A _DataSet_ is treated internally as a stream of data. The concepts above thus apply to batch programs in the same way as well as they apply to streaming programs, with minor exceptions:
  ...
  * The DataSet API introduces special synchronized (superstep-based) iterations, which are only possible on bounded streams. For details, check out the [iteration docs](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/dev/dataset/iterations/>).
