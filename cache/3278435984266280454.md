...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/table/common/>).
...
**注意：** Table API 和 SQL 查询可以很容易地集成并嵌入到 DataStream 程序中。 请参阅[与 DataStream API 集成](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/data_stream_api/>) 章节了解如何将 DataStream 与表之间的相互转化。
...
有关 `TableEnvironment` 所有方法的完整参考，请参阅 [TableEnvironment API](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/table_environment/>) 页面。
...
永久表需要 [catalog](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/catalogs/>)（例如 Hive Metastore）以维护表的元数据。一旦永久表被创建，它将对任何连接到 catalog 的 Flink 会话可见且持续存在，直至被明确删除。
...
另外一个方式去创建 `TABLE` 是通过 [connector](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/connectors/table/overview/>) 声明。Connector 描述了存储表数据的外部系统。存储系统例如 Apache Kafka 或者常规的文件系统都可以通过这种方式来声明。
...
文档 [Table API](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/tableapi/>) 说明了所有流处理和批处理表支持的 Table API 算子。
...
文档 [SQL](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/overview/>) 描述了Flink对流处理和批处理表的SQL支持。
...
请参考文档 [Table Sources & Sinks](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/sourcessinks/>) 以获取更多关于可用 Sink 的信息以及如何自定义 `DynamicTableSink`。
...
不论输入数据源是流式的还是批式的，Table API 和 SQL 查询都会被转换成 [DataStream](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/datastream/overview/>) 程序。 查询在内部表示为逻辑查询计划，并被翻译成两个阶段：
...
可以用 `TableEnvironment.explainSql()` 方法和 `TableEnvironment.executeSql()` 方法支持执行一个 `EXPLAIN` 语句获取逻辑和优化查询计划，请参阅 [EXPLAIN](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/utility/explain/>) 页面.
