...
Every function and operator in Flink can be **stateful** (see [working with state](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/concepts/stateful-stream-processing/>) for details). Stateful functions store data across the processing of individual elements/events, making state a critical building block for any type of more elaborate operation.
...
The [documentation on streaming fault tolerance](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/learn-flink/fault_tolerance/>) describes in detail the technique behind Flink’s streaming fault tolerance mechanism.
...
By default, checkpointing is disabled. To enable checkpointing, call `enableCheckpointing(n)` on the `StreamExecutionEnvironment`, where _n_ is the [checkpoint interval](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/ops/production_ready/#choose-the-right-checkpoint-interval>) in milliseconds.
...
Some more parameters and/or defaults may be set via Flink configuration file (see [configuration](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/deployment/config/>))` for a full guide):
...
Flink’s [checkpointing mechanism](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/learn-flink/fault_tolerance/>) stores consistent snapshots of all the state in timers and stateful operators, including connectors, windows, and any [user-defined state](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/datastream/fault-tolerance/state/>). Where the checkpoints are stored (e.g., JobManager memory, file system, database) depends on the configured **Checkpoint Storage**.
...
See [checkpoint storage](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/ops/state/checkpoints/#checkpoint-storage>) for more details on the available options for job-wide and cluster-wide configuration.
