...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/table/hive-compatibility/hive-dialect/queries/lateral-view/>).

#  Lateral View Clause #

##  Description #

Lateral view clause is used in conjunction with user-defined table generating functions([UDTF](<https://cwiki.apache.org/confluence/display/Hive/LanguageManual+UDF#LanguageManualUDF-Built-inTable-GeneratingFunctions%28UDTF%29>)) such as `explode()`. A UDTF generates zero or more output rows for each input row.
