...
如果遇到问题，可以参考 [社区支持资源](<https://flink.apache.org/community.html>)。 Flink 的 [用户邮件列表](<https://flink.apache.org/community.html#mailing-lists>) 是 Apache 项目中最活跃的一个，这也是快速寻求帮助的重要途径。
...
Flink 支持使用纯 [SQL](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/sql/reference/overview/>) 或者 [Table API](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/table/tableapi/>) 开发关系型数据应用。Table API 是受 SQL 启发设计出的一套链式 DSL，可以用 Java 或 Python 开发，在 IDE 中也集成的很好。同时也如 SQL 查询一样，Table 应用可以按列查询，或者按列分组。通过类似 `floor` 以及 `sum` 这样的 [系统函数](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/sql/functions/built-in-functions/>)，你已经可以开发这个报表了。
...
Flink 内置的函数是有限的，有时是需要通过 [用户自定义函数](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/table/functions/udfs/>)来拓展这些函数。假如 `floor` 函数不是系统预设函数，也可以自己实现。
...
对于更高级的逐行处理，Flink 提供了 [Process Table Functions](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/table/functions/ptfs/>)（PTFs）。PTFs 可以转换表的每一行，并且能够访问强大的功能，如状态和定时器。下面是一个简单的无状态示例，用于过滤和格式化高额交易：
...
这将只输出超过 500 的交易警报。当 PTFs 与状态和定时器结合使用时，可以实现复杂的事件驱动逻辑——更多高级示例请参阅 [PTF 文档](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/table/functions/ptfs/>)。

> **注意：** Process Table Functions 目前仅在 Java 中可用。对于 Python，可以使用 [用户自定义表函数](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/table/functions/udfs/#table-functions>)（UDTFs）来实现类似的逐行处理。

##  添加窗口函数 #

在数据处理中，按照时间做分组是常见操作，在处理无限流时更是如此。按时间分组的函数叫 [window](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/table/tableapi/#groupby-window-aggregation>)，Flink 提供了灵活的窗口函数语法。最常见的窗口是 `Tumble`，窗口区间长度固定，并且区间不重叠。
...
上述命令会构建 Python Table API 程序，并在本地 mini cluster 中运行。如果想将作业提交到远端集群执行，可以参考[作业提交示例](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/cli/#submitting-pyflink-jobs>)。
...
###  深入了解 Table API #
  ...
  * [Table API 概述](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/table/tableapi/>)：完整的 Table API 参考
  * [用户自定义函数](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/table/functions/udfs/>)：为你的流水线创建自定义函数
  * [流式概念](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/concepts/sql-table-concepts/overview/>)：了解动态表、时间属性等概念

###  探索其他教程 #

  * [Flink SQL 教程](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/getting-started/quickstart-sql/>)：无需编码的交互式 SQL 查询
  * [DataStream API 教程](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/getting-started/datastream/>)：使用 DataStream API 构建有状态流处理应用
  * [Flink 运维练习场](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/getting-started/flink-operations-playground/>)：学习操作 Flink 集群

###  生产部署 #

  * [部署概述](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/overview/>)：在生产环境部署 Flink
  * [连接器](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/connectors/table/overview/>)：连接 Kafka、数据库、文件系统等