...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/learn-flink/streaming_analytics/>).
...
If you want to use event time, you will also need to supply a Timestamp Extractor and Watermark Generator that Flink will use to track the progress of event time. This will be covered in the section below on [Working with Watermarks](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/learn-flink/streaming_analytics/#working-with-watermarks>), but first we should explain what watermarks are.
...
Event time processing in Flink depends on _watermark generators_ that insert special timestamped elements into the stream, called _watermarks_. A watermark for time _t_ is an assertion that the stream is (probably) now complete up through time _t_.
...
Each event arrives after some delay, and these delays vary, so some events are delayed more than others. One simple approach is to assume that these delays are bounded by some maximum delay. Flink refers to this strategy as _bounded-out-of-orderness_ watermarking. It is easy to imagine more complex approaches to watermarking, but for most applications a fixed delay works well enough.
...
You can arrange for the events that would be dropped to be collected to an alternate output stream instead, using a mechanism called [Side Outputs](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/learn-flink/event_driven/#side-outputs>). Here is an example of what that might look like:
...
Note, however, that the tumbling and sliding window assigners take an optional offset parameter that can be used to change the alignment of the windows. See [Tumbling Windows](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/datastream/operators/windows/#tumbling-windows>) and [Sliding Windows](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/datastream/operators/windows/#sliding-windows>) for details.
...
The hands-on exercise that goes with this section is the [ Hourly Tips Exercise ](<//github.com/apache/flink-training/blob/release-1.19//hourly-tips>) .

##  Further Reading #

  * [Timely Stream Processing](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/concepts/time/>)
  * [Windows](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/datastream/operators/windows/>)
