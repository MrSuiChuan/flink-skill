...
Flink allows reporting traces to external systems. For more information about Flink’s tracing system go to the [tracing system documentation](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/ops/traces/>).

Traces can be exposed to an external system by configuring one or several reporters in [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/deployment/config/#flink-configuration-file>). These reporters will be instantiated on each job and task manager when they are started.
...
**Important:** The jar containing the reporter must be accessible when Flink is started. Reporters are loaded as [plugins](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/deployment/filesystems/plugins/>). All reporters documented on this page are available by default.
