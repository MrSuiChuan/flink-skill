...
> Table API and SQL queries can be easily integrated with and embedded into DataStream programs. Have a look at the [DataStream API Integration](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/table/data_stream_api/>) page to learn how DataStreams can be converted into Tables and vice versa.
...
Permanent tables require a [catalog](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/table/catalogs/>) (such as Hive Metastore) to maintain metadata about the table. Once a permanent table is created, it is visible to any Flink session that is connected to the catalog and will continue to exist until the table is explicitly dropped.
...
The [Table API](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/table/tableapi/>) document describes all Table API operations that are supported on streaming and batch tables.
...
The [SQL](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/table/sql/overview/>) document describes Flink’s SQL support for streaming and batch tables.
...
Please see the documentation about [Table Sources & Sinks](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/table/sourcessinks/>) for details about available sinks and instructions for how to implement a custom `DynamicTableSink`.
...
The behavior of translating and executing a query is different for the two planners. Table API and SQL queries are translated into [DataStream](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/overview/>) programs whether their input is streaming or batch. A query is internally represented as a logical query plan and is translated in two phases:
...
`TableEnvironment.explainSql()` and `TableEnvironment.executeSql()` support execute a `EXPLAIN` statement to get the plans, Please refer to [EXPLAIN](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/table/sql/explain/>) page.
