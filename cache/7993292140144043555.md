#  [Managing Large State in Apache Flink: An Intro to Incremental Checkpointing](</2018/01/30/managing-large-state-in-apache-flink-an-intro-to-incremental-checkpointing/>)

January 30, 2018 - Stefan Ricther [(@StefanRRicther)](<https://twitter.com/StefanRRicther>) Chris Ward [(@chrischinch)](<https://twitter.com/chrischinch>)
...
State is a fundamental, enabling concept in stream processing required for a majority of complex use cases. Some examples highlighted in the [Flink documentation](<//nightlies.apache.org/flink/flink-docs-release-1.3/dev/stream/state.html>):
...
To enable incremental checkpointing in your application, I recommend you read the [the Apache Flink documentation on checkpointing](<//nightlies.apache.org/flink/flink-docs-release-1.4/ops/state/large_state_tuning.html#tuning-rocksdb>) for full details, but in summary, you enable checkpointing as normal, but enable incremental checkpointing in the constructor by setting the second parameter to `true`.
...
By default, Flink retains 1 completed checkpoint, so if you need a higher number, [you can configure it with the following flag](<//nightlies.apache.org/flink/flink-docs-master/dev/stream/state/checkpointing.html#related-config-options>):
...
There are some strategies for improving the convenience/performance trade-off, and I recommend you read [the Flink documentation](<//nightlies.apache.org/flink/flink-docs-release-1.4/ops/state/checkpoints.html#basics-of-incremental-checkpoints>) for more details.

_This post[ originally appeared on the data Artisans blog ](<https://data-artisans.com/blog/managing-large-state-apache-flink-incremental-checkpointing-overview>)and was contributed to the Flink blog by Stefan Richter and Chris Ward._

[Want to contribute translation?](<https://cwiki.apache.org/confluence/display/FLINK/Flink+Translation+Specifications>)

[ Edit This Page __](<//github.com/apache/flink-web/edit/asf-site/docs/content/posts/2018-01-30-incremental-checkpointing.md>)