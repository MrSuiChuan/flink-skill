...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/sql/materialized-table/statements/>).
...
`WITH Options` 可以定义创建物化表所需的属性，包括[连接器参数](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/connectors/table/>)和分区字段的[时间格式参数](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/config/#partition-fields-date-formatter>)。
...
注意
  ...
  * [partition.fields.#.date-formatter](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/config/#partition-fields-date-formatter>) 选项仅适用于全量模式。
  * [partition.fields.#.date-formatter](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/config/#partition-fields-date-formatter>) 中的字段必须是有效的字符串类型分区字段。
...
数据新鲜度定义了物化表内容滞后于基础表更新的最长时间。When not specified, it uses the default value from configuration based on the refresh mode. 它有两个作用，首先通过[配置](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/config/#materialized-table-refresh-mode-freshness-threshold>)确定物化表的[刷新模式](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/materialized-table/overview/#%e5%88%b7%e6%96%b0%e6%a8%a1%e5%bc%8f>)，然后确定数据刷新频率以满足实际数据新鲜度要求。

**FRESHNESS 参数详解**

FRESHNESS 参数的取值范围为 INTERVAL `'<num>'` { SECOND | MINUTE | HOUR | DAY }。`'<num>'` 必须为正整数，并且在全量模式下，`'<num>'` 应该是相应时间间隔单位的公约数。

**示例：** （假定 `materialized-table.refresh-mode.freshness-threshold` 为 30 分钟）
...
注意
  ...
  * 在持续模式下，数据新鲜度和 `checkpoint` 间隔一致，设置过短的数据新鲜度可能会对作业性能产生影响。此外，为了优化 `checkpoint` 性能，建议[开启 Changelog](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/state_backends/#%e5%bc%80%e5%90%af-changelog>)。
...
该子句用于定义填充物化表数据的查询。上游表可以是物化表、表或视图。select 语句支持所有 Flink SQL [查询](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/queries/overview/>)。
...
Use `ADD` clause to add [columns](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/ddl/create/#columns>) (only non persisted like computed and metadata virtual), [constraints](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/ddl/create/#primary-key>), a [watermark](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/ddl/create/#watermark>), and a [distribution](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/ddl/create/#distributed>) to an existing materialized table.
...
Use `MODIFY` clause to change column’s comment, position, type (only non persisted like computed and metadata virtual also see [columns](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/ddl/create/#columns>)), change primary key columns and watermark strategy to an existing table.
...
Use the `DROP` clause to drop columns (only non persisted like computed and metadata virtual also see [columns](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/ddl/create/#columns>)), primary key, partitions, and watermark strategy to an existing table.
...
注意
  ...
  * 暂停持续模式的表时，默认会使用 `STOP WITH SAVEPOINT` 暂停作业，你需要使用[参数](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#execution-checkpointing-savepoint-dir>)设置 `SAVEPOINT` 保存路径。
...
**全量模式：**
  ...
  2. 在刷新作业下次触发执行时，将使用新的查询定义刷新数据:
     * 如果修改的物化表是分区表，且[partition.fields.#.date-formatter](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/config/#partition-fields-date-formatter>) 配置正确，则仅刷新最新分区。
     * 否则，将刷新整个表的数据。
...
**持续模式：**
  ...
  3. 启动新的流式作业以刷新物化表：
     ...
     * 数据源的起始位点会由到连接器的默认实现或查询中设置的 [dynamic hint](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/queries/hints/#dynamic-table-options>) 决定。
