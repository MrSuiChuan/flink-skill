...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/sql/reference/utility/analyze/>).
...
`ANALYZE TABLE` statements can be executed in [SQL CLI](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/interfaces/sql-client/>).
...
_NOTE:_ For the fix length types (like `BOOLEAN`, `INTEGER`, `DOUBLE` etc.), we need not collect the `avgLen` and `maxLen` from the original records.
