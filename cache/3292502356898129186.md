...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/ops/state/state_backends/>).

#  State Backends #

用 [Data Stream API](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/datastream/overview/>) 编写的程序通常以各种形式保存状态：

  * 在 Window 触发之前要么收集元素、要么聚合
  * 转换函数可以使用 key/value 格式的状态接口来存储状态
  * 转换函数可以实现 `CheckpointedFunction` 接口，使其本地变量具有容错能力

另请参阅 Streaming API 指南中的 [状态部分](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/datastream/fault-tolerance/state/>) 。
...
建议同时将 [managed memory](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/memory/mem_setup_tm/#managed-memory>) 设为0，以保证将最大限度的内存分配给 JVM 上的用户代码。
...
请同时参考 [Task Executor 内存配置](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/memory/mem_tuning/#rocksdb-state-backend>) 中关于 EmbeddedRocksDBStateBackend 的建议。

EmbeddedRocksDBStateBackend 是目前唯一支持增量 CheckPoint 的 State Backend (见 [这里](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/large_state_tuning/>))。

可以使用一些 RocksDB 的本地指标(metrics)，但默认是关闭的。你能在 [这里](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#rocksdb-native-metrics>) 找到关于 RocksDB 本地指标的文档。

每个 slot 中的 RocksDB instance 的内存大小是有限制的，详情请见 [这里](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/large_state_tuning/>)。
...
如果没有明确指定，将使用 jobmanager 做为默认的 state backend。你能在 [**Flink 配置文件**](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#flink-配置文件>) 中为所有 Job 设置其他默认的 State Backend。 每一个 Job 的 state backend 配置会覆盖默认的 state backend 配置，如下所示：
...
> **注意:** 由于 RocksDB 是 Flink 默认分发包的一部分，所以如果你没在代码中使用 RocksDB，则不需要添加此依赖。而且可以在 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#flink-%e9%85%8d%e7%bd%ae%e6%96%87%e4%bb%b6>)中通过 `state.backend.type` 配置 State Backend，以及更多的 [checkpointing](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#checkpointing>) 和 [RocksDB 特定的](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#rocksdb-state-backend>) 参数。

###  设置默认的（全局的） State Backend #

在 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#flink-配置文件>) 可以通过键 `state.backend.type` 设置默认的 State Backend。

可选值包括 _jobmanager_ (HashMapStateBackend), _rocksdb_ (EmbeddedRocksDBStateBackend)， 或使用实现了 state backend 工厂 [ StateBackendFactory ](<//github.com/apache/flink/blob/release-1.20/flink-runtime/src/main/java/org/apache/flink/runtime/state/StateBackendFactory.java>) 的类的全限定类名， 例如： EmbeddedRocksDBStateBackend 对应为 `org.apache.flink.contrib.streaming.state.EmbeddedRocksDBStateBackendFactory`。

`execution.checkpointing.dir` 选项指定了所有 State Backend 写 CheckPoint 数据和写元数据文件的目录。 你能在 [这里](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/checkpoints/#directory-structure>) 找到关于 CheckPoint 目录结构的详细信息。
...
虽然状态数据量很大时我们推荐使用增量快照，但这并不是默认的快照机制，您需要通过下述配置手动开启该功能：
  ...
  * 在 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#flink-配置文件>) 中设置：`execution.checkpointing.incremental: true` 或者
...
当然，您也可以选择不使用 Flink 自带的内存管理，而是手动为 RocksDB 的每个列族（ColumnFamily）分配内存（每个算子的每个 state 都对应一个列族）。这为专业用户提供了对 RocksDB 进行更细粒度控制的途径，但同时也意味着用户需要自行保证总内存消耗不会超过（尤其是容器）环境的限制。请参阅 [large state tuning](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/large_state_tuning/#tuning-rocksdb-memory>) 了解有关大状态数据性能调优的一些指导原则。
...
您可以选择使用 Flink 的监控指标系统来汇报 RocksDB 的原生指标，并且可以选择性的指定特定指标进行汇报。 请参阅 [configuration docs](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#rocksdb-native-metrics>) 了解更多详情。
...
有两种方法可以选择要应用的预定义选项：
  ...
  * 通过 `state.backend.rocksdb.predefined-options` 配置项将选项名称设置进 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#flink-配置文件>) 。
...
RocksDB State Backend 会将 [这里定义](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#advanced-rocksdb-state-backends-options>) 的所有配置项全部加载。 因此您可以简单的通过关闭 RocksDB 使用托管内存的功能并将需要的设置选项加入配置文件来配置底层的列族选项。
...
有两种方法可以将 `RocksDBOptionsFactory` 传递给 RocksDB State Backend：
  ...
  * 通过 `state.backend.rocksdb.options-factory` 选项将工厂实现类的名称设置到[Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#flink-配置文件>) 。

  * 通过程序设置，例如 `EmbeddedRocksDBStateBackend.setRocksDBOptions(new MyOptionsFactory());` 。

注意 通过程序设置的 `RocksDBOptionsFactory` 将覆盖 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#flink-配置文件>) 的设置，且 `RocksDBOptionsFactory` 设置的优先级高于预定义选项（`PredefinedOptions`）。
...
一般情况下 checkpoint 的持续时间受如下因素影响：
  ...
  1. Barrier 到达和对齐时间，可以通过 [Unaligned checkpoints](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/checkpointing_under_backpressure/#unaligned-checkpoints>) 和 [Buffer debloating](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/checkpointing_under_backpressure/#buffer-debloating>) 解决。
...
请确保[添加](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/filesystems/overview/>)所需的文件系统插件。
...
有关其他配置选项，请参阅[配置](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#state-changelog-options>)部分。
...
[此处](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/metrics/#state-changelog>)列出了可用的指标。
...
支持从 savepoint 或 checkpoint 恢复：
  ...
  * 创建一个 [savepoint](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/savepoints/#resuming-from-savepoints>) 或一个 [checkpoint](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/checkpoints/#resuming-from-a-retained-checkpoint>)
  * 更改配置（开启 Changelog）
  * 从创建的 snapshot 恢复
...
###  限制 #
  ...
  * 尚不支持 [NO_CLAIM](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#execution-savepoint-restore-mode>) 模式
...
旧版本的 `MemoryStateBackend` 等价于使用 `HashMapStateBackend` 和 [`JobManagerCheckpointStorage`](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/checkpoints/#the-jobmanagercheckpointstorage>)。
...
旧版本的 `FsStateBackend` 等价于使用 `HashMapStateBackend` 和 [`FileSystemCheckpointStorage`](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/checkpoints/#the-filesystemcheckpointstorage>)。
...
旧版本的 `RocksDBStateBackend` 等价于使用 `EmbeddedRocksDBStateBackend` 和 [`FileSystemCheckpointStorage`](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/state/checkpoints/#the-filesystemcheckpointstorage>).
