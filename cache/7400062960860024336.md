...
In order to create your own Flink DataStream program, we encourage you to start with anatomy of a Flink Program and gradually add your own [stream transformations](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/operators/overview/>). The remaining sections act as references for additional operations and advanced features.
...
We will now give an overview of each of those steps, please refer to the respective sections for more details. Note that all core classes of the Java DataStream API can be found in [ org.apache.flink.streaming.api ](<//github.com/apache/flink/blob/release-1.13//flink-streaming-java/src/main/java/org/apache/flink/streaming/api>)
...
We will now give an overview of each of those steps, please refer to the respective sections for more details. Note that all core classes of the Scala DataStream API can be found in [ org.apache.flink.streaming.api.scala ](<//github.com/apache/flink/blob/release-1.13//flink-streaming-scala/src/main/scala/org/apache/flink/streaming/api/scala>)
...
Custom:
  ...
  * `addSource` \- Attach a new source function. For example, to read from Apache Kafka you can use `addSource(new FlinkKafkaConsumer<>(...))`. See [connectors](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/connectors/datastream/overview/>) for more details.
...
Please see [operators](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/operators/overview/>) for an overview of the available stream transformations.
...
Iterative streaming programs implement a step function and embed it into an `IterativeStream`. As a DataStream program may never finish, there is no maximum number of iterations. Instead, you need to specify which part of the stream is fed back to the iteration and which part is forwarded downstream using a [side output](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/side_output/>) or a `filter`. Here, we show an example using filters. First, we define an `IterativeStream`
...
Please refer to [execution configuration](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/deployment/config/>) for an explanation of most parameters. These parameters pertain specifically to the DataStream API:
...
[State & Checkpointing](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/fault-tolerance/checkpointing/>) describes how to enable and configure Flink’s checkpointing mechanism.
...
##  Where to go next? #
  ...
  * [Operators](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/operators/overview/>): Specification of available streaming operators.
  * [Event Time](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/concepts/time/>): Introduction to Flink’s notion of time.
  * [State & Fault Tolerance](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/fault-tolerance/state/>): Explanation of how to develop stateful applications.
  * [Connectors](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/connectors/datastream/overview/>): Description of available input and output connectors.
