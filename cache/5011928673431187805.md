...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/ops/state/state_backends/>).

#  State Backends #

用 [Data Stream API](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/datastream/overview/>) 编写的程序通常以各种形式保存状态：

  * 在 Window 触发之前要么收集元素、要么聚合
  * 转换函数可以使用 key/value 格式的状态接口来存储状态
  * 转换函数可以实现 `CheckpointedFunction` 接口，使其本地变量具有容错能力

另请参阅 Streaming API 指南中的 [状态部分](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/datastream/fault-tolerance/state/>) 。
...
在 _HashMapStateBackend_ 内部，数据以 Java 对象的形式存储在堆中。 Key/value 形式的状态和窗口算子会持有一个 hash table，其中存储着状态值、触发器。
...
建议同时将 [managed memory](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/memory/mem_setup_tm/#managed-memory>) 设为0，以保证将最大限度的内存分配给 JVM 上的用户代码。
...
请同时参考 [Task Executor 内存配置](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/memory/mem_tuning/#rocksdb-state-backend>) 中关于 EmbeddedRocksDBStateBackend 的建议。

EmbeddedRocksDBStateBackend 是目前唯一支持增量 CheckPoint 的 State Backend (见 [这里](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/large_state_tuning/>))。

可以使用一些 RocksDB 的本地指标(metrics)，但默认是关闭的。你能在 [这里](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#rocksdb-native-metrics>) 找到关于 RocksDB 本地指标的文档。

每个 slot 中的 RocksDB instance 的内存大小是有限制的，详情请见 [这里](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/large_state_tuning/>)。

###  The ForStStateBackend #

The _ForStStateBackend_ is a state backend that is based on [ForSt project](<https://github.com/ververica/ForSt>), which is also a LSM-tree structured key-value store and built on top of the RocksDB. It is designed for disaggregated state management, for more details, see [here](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/disaggregated_state/>). Most importantly, it can hold its sst files on remote file systems that Flink supports, such as HDFS, S3, etc. This allows Flink to scale the state size beyond the local disk capacity of the TaskManager. Moreover, by putting the sst files on remote file systems, it can also provide a more lightweight way to perform checkpoint and recovery.
...
> 在 Flink 1.13 版本中我们统一了 savepoints 的二进制格式。这意味着你可以生成 savepoint 并且之后使用另一种 state backend 读取它。 从 1.13 版本开始，所有的 state backends 都会生成一种普适的格式。因此，如果想切换 state backend 的话，那么最好先升级你的 Flink 版本，在新版本中生成 savepoint，在这之后你才可以使用一个不同的 state backend 来读取并恢复它。

##  设置 State Backend #

如果没有明确指定，将使用 `HashMapStateBackend` 做为默认的 state backend。你能在 [**Flink 配置文件**](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#flink-配置文件>) 中为所有 Job 设置其他默认的 State Backend。 每一个 Job 的 state backend 配置会覆盖默认的 state backend 配置，如下所示：
...
> **注意:** 由于 RocksDB 是 Flink 默认分发包的一部分，所以如果你没在代码中使用 RocksDB，则不需要添加此依赖。而且可以在 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#flink-%e9%85%8d%e7%bd%ae%e6%96%87%e4%bb%b6>)中通过 `state.backend.type` 配置 State Backend，以及更多的 [checkpointing](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#checkpointing>) 和 [RocksDB 特定的](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#rocksdb-state-backend>) 参数。

###  设置默认的（全局的） State Backend #

在 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#flink-配置文件>) 可以通过键 `state.backend.type` 设置默认的 State Backend。

可选值包括 _hashmap_ (HashMapStateBackend), _rocksdb_ (EmbeddedRocksDBStateBackend)， 或使用实现了 state backend 工厂 [ StateBackendFactory ](<//github.com/apache/flink/blob/master/flink-runtime/src/main/java/org/apache/flink/runtime/state/StateBackendFactory.java>) 的类的全限定类名， 例如： EmbeddedRocksDBStateBackend 对应为 `org.apache.flink.state.rocksdb.EmbeddedRocksDBStateBackendFactory`。

`execution.checkpointing.dir` 选项指定了所有 State Backend 写 CheckPoint 数据和写元数据文件的目录。 你能在 [这里](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/checkpoints/#directory-structure>) 找到关于 CheckPoint 目录结构的详细信息。
...
_该小节描述 RocksDB state backend 的更多细节_

###  增量快照 #

RocksDB 支持 _增量快照_ 。不同于产生一个包含所有数据的全量备份，增量快照中只包含自上一次快照完成之后被修改的记录，因此可以显著减少快照完成的耗时。
...
虽然状态数据量很大时我们推荐使用增量快照，但这并不是默认的快照机制，您需要通过下述配置手动开启该功能：
  ...
  * 在 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#flink-配置文件>) 中设置：`execution.checkpointing.incremental: true` 或者
...
当然，您也可以选择不使用 Flink 自带的内存管理，而是手动为 RocksDB 的每个列族（ColumnFamily）分配内存（每个算子的每个 state 都对应一个列族）。这为专业用户提供了对 RocksDB 进行更细粒度控制的途径，但同时也意味着用户需要自行保证总内存消耗不会超过（尤其是容器）环境的限制。请参阅 [large state tuning](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/large_state_tuning/#tuning-rocksdb-memory>) 了解有关大状态数据性能调优的一些指导原则。
...
Flink还提供了两个参数来控制 _写路径_ （MemTable）和 _读路径_ （索引及过滤器，读缓存）之间的内存分配。当您看到 RocksDB 由于缺少写缓冲内存（频繁刷新）或读缓存未命中而性能不佳时，可以使用这些参数调整读写间的内存分配。
...
注意 _仅面向专业用户_ ：若要手动控制内存，可以将 `state.backend.rocksdb.memory.managed` 设置为 `false`，并通过 `ColumnFamilyOptions` 配置 RocksDB。 或者可以复用上述 cache/write-buffer-manager 机制，但将内存大小设置为与 Flink 的托管内存大小无关的固定大小（通过 `state.backend.rocksdb.memory.fixed-per-slot`/`state.backend.rocksdb.memory.fixed-per-tm` 选项）。 注意在这两种情况下，用户都需要确保在 JVM 之外有足够的内存可供 RocksDB 使用。
...
注意 _在 RocksDB state backend 中使用基于堆的计时器的组合当前不支持计时器状态的异步快照。其他状态（如 keyed state）可以被异步快照。_

###  开启 RocksDB 原生监控指标 #

您可以选择使用 Flink 的监控指标系统来汇报 RocksDB 的原生指标，并且可以选择性的指定特定指标进行汇报。 请参阅 [configuration docs](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#rocksdb-native-metrics>) 了解更多详情。

> **注意：** 启用 RocksDB 的原生指标可能会对应用程序的性能产生负面影响。

####  列族（ColumnFamily）级别的预定义选项 #

注意 在引入 RocksDB 使用托管内存 功能后，此机制应限于在 _专家调优_ 或 _故障处理_ 中使用。

使用 _预定义选项_ ，用户可以在每个 RocksDB 列族上应用一些预定义的配置，例如配置内存使用、线程、Compaction 设置等。目前每个算子的每个状态都在 RocksDB 中有专门的一个列族存储。

有两种方法可以选择要应用的预定义选项：

  * 通过 `state.backend.rocksdb.predefined-options` 配置项将选项名称设置进 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#flink-配置文件>) 。
...
RocksDB State Backend 会将 [这里定义](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#advanced-rocksdb-state-backends-options>) 的所有配置项全部加载。 因此您可以简单的通过关闭 RocksDB 使用托管内存的功能并将需要的设置选项加入配置文件来配置底层的列族选项。
...
有两种方法可以将 `RocksDBOptionsFactory` 传递给 RocksDB State Backend：
  ...
  * 通过 `state.backend.rocksdb.options-factory` 选项将工厂实现类的名称设置到[Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#flink-配置文件>) 。

  * 通过程序设置，例如 `EmbeddedRocksDBStateBackend.setRocksDBOptions(new MyOptionsFactory());` 。

注意 通过程序设置的 `RocksDBOptionsFactory` 将覆盖 [Flink 配置文件](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#flink-配置文件>) 的设置，且 `RocksDBOptionsFactory` 设置的优先级高于预定义选项（`PredefinedOptions`）。
...
一般情况下 checkpoint 的持续时间受如下因素影响：
  ...
  1. Barrier 到达和对齐时间，可以通过 [Unaligned checkpoints](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/checkpointing_under_backpressure/#unaligned-checkpoints>) 和 [Buffer debloating](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/checkpointing_under_backpressure/#buffer-debloating>) 解决。

  2. 快照制作时间（所谓同步阶段）, 可以通过异步快照解决（如上文所述）。
...
值得注意的是虽然 Changelog 增加了少量的日常 CPU 和网络带宽资源使用， 但会降低峰值的 CPU 和网络带宽使用量。
...
请确保[添加](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/filesystems/overview/>)所需的文件系统插件。
...
有关其他配置选项，请参阅[配置](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#state-changelog-options>)部分。
...
[此处](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/metrics/#state-changelog>)列出了可用的指标。
...
支持从 savepoint 或 checkpoint 恢复：
  ...
  * 创建一个 [savepoint](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/savepoints/#resuming-from-savepoints>) 或一个 [checkpoint](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/checkpoints/#resuming-from-a-retained-checkpoint>)
  * 更改配置（开启 Changelog）
  * 从创建的 snapshot 恢复
...
###  限制 #
  ...
  * 尚不支持 [NO_CLAIM](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/deployment/config/#execution-savepoint-restore-mode>) 模式
...
旧版本的 `MemoryStateBackend` 等价于使用 `HashMapStateBackend` 和 [`JobManagerCheckpointStorage`](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/checkpoints/#the-jobmanagercheckpointstorage>)。
...
旧版本的 `FsStateBackend` 等价于使用 `HashMapStateBackend` 和 [`FileSystemCheckpointStorage`](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/checkpoints/#the-filesystemcheckpointstorage>)。
...
旧版本的 `RocksDBStateBackend` 等价于使用 `EmbeddedRocksDBStateBackend` 和 [`FileSystemCheckpointStorage`](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/ops/state/checkpoints/#the-filesystemcheckpointstorage>).
