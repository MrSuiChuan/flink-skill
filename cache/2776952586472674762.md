...
You can use the [command line client](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/cli/#savepoints>) to _trigger savepoints_ , _cancel a job with a savepoint_ , _resume from savepoints_ , and _dispose savepoints_.
...
>
>   1. if [_entropy injection_](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/filesystems/s3/#entropy-injection-for-s3-file-systems>) is activated: In that case the savepoint directory will not contain all savepoint data files, because the injected path entropy spreads the files over many directories. Lacking a common savepoint root directory, the savepoints will contain absolute path references, which prevent moving the directory.
>
...
When using this command, the client returns immediately after getting the trigger id of the savepoint. You can monitor the status of the savepoint through the REST API [rest api](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/ops/rest_api//#jobs-jobid-checkpoints-triggerid>).
