...
---|---
6.x |  There is no connector (yet) available for Flink version 2.3.
7.x |  There is no connector (yet) available for Flink version 2.3.
8.x |  There is no connector (yet) available for Flink version 2.3.

In order to use the in PyFlink jobs, the following dependencies are required:

Version | PyFlink JAR | flink-connector-elasticsearch6 | There is no SQL jar (yet) available for Flink version 2.3.
---|---
flink-connector-elasticsearch7 | There is no SQL jar (yet) available for Flink version 2.3.
...
For the execution of your Flink program, it is recommended to build a so-called uber-jar (executable jar) containing all your dependencies (see [here](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/dev/configuration/>) for further information).
