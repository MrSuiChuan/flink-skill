...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/concepts/stateful-stream-processing/>).
...
Flink needs to be aware of the state in order to make it fault tolerant using [checkpoints](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/datastream/fault-tolerance/checkpointing/>) and [savepoints](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/savepoints/>).
...
When working with state, it might also be useful to read about [Flink’s state backends](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/state_backends/>). Flink provides different state backends that specify how and where state is stored.
...
See [Restart Strategies](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/task_failure_recovery/#restart-strategies>) for more information.
...
Unaligned checkpointing ensures that barriers are arriving at the sink as fast as possible. It’s especially suited for applications with at least one slow moving data path, where alignment times can reach hours. However, since it’s adding additional I/O pressure, it doesn’t help when the I/O to the state backends is the bottleneck. See the more in-depth discussion in [ops](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/checkpoints/#unaligned-checkpoints>) for other limitations.
...
[Savepoints](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/savepoints/>) are **manually triggered checkpoints** , which take a snapshot of the program and write it out to a state backend. They rely on the regular checkpointing mechanism for this.

Savepoints are similar to checkpoints except that they are **triggered by the user** and **don’t automatically expire** when newer checkpoints are completed. To make proper use of savepoints, it’s important to understand the differences between [checkpoints](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/checkpoints/>) and [savepoints](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/savepoints/>) which is described in [checkpoints vs. savepoints](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/checkpoints_vs_savepoints/>).
