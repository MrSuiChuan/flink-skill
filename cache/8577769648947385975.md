...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/deployment/advanced/logging/>).

#  How to use logging #

All Flink processes create a log text file that contains messages for various events happening in that process. These logs provide deep insights into the inner workings of Flink, and can be used to detect problems (in the form of WARN/ERROR messages) and can help in debugging them.

The log files can be accessed via the Job-/TaskManager pages of the WebUI. The used [Resource Provider](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/deployment/overview/>) (e.g., YARN) may provide additional means of accessing them.

The logging in Flink uses the [SLF4J](<http://www.slf4j.org/>) logging interface. This allows you to use any logging framework that supports SLF4J, without having to modify the Flink source code.
...
The following [logging-related configuration options](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/deployment/config/#jvm-and-logging-options>) are available:
