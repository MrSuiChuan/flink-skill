...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/sql/materialized-table/statements/>).
...
`WITH Options` are used to specify the materialized table properties, including [connector options](<//nightlies.apache.org/flink/flink-docs-master/docs/connectors/table/>) and [time format option](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/config/#partition-fields-date-formatter>) for partition fields.
...
Note
  ...
  * The [partition.fields.#.date-formatter](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/config/#partition-fields-date-formatter>) option only works in full mode.
  * The field in the [partition.fields.#.date-formatter](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/config/#partition-fields-date-formatter>) must be a valid string type partition field.
...
FRESHNESS defines the maximum amount of time that the materialized table’s content should lag behind updates to the base tables. When not specified, it uses the default value from configuration based on the refresh mode. It does two things: firstly it determines the [refresh mode](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/materialized-table/overview/#refresh-mode>) of the materialized table through [configuration](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/config/#materialized-table-refresh-mode-freshness-threshold>), followed by determining the data refresh frequency to meet the actual data freshness requirements.
...
Note
  ...
  * In CONTINUOUS mode, setting a data freshness interval that is too short can impact job performance as it aligns with the checkpoint interval. To optimize checkpoint performance, consider [enabling-changelog](<//nightlies.apache.org/flink/flink-docs-master/docs/ops/state/state_backends/#incremental-checkpoints>).
...
This clause is used to define the query for populating materialized table data. The upstream table can be a materialized table, table, or view. The select statement supports all Flink SQL [Queries](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/reference/queries/overview/>).
...
The `OR ALTER` clause provides create-or-update semantics:
  ...
  * **If a regular table with the same name exists** : Converts it in place into a materialized table, when enabled (see Converting a Table to a Materialized Table)
...
The operation updates the materialized table similarly to ALTER MATERIALIZED TABLE AS:
...
**Using the converted table as a regular table**
...
Use `ADD` clause to add [columns](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/reference/ddl/create/#columns>) (only non persisted like computed and metadata virtual), [constraints](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/reference/ddl/create/#primary-key>), a [watermark](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/reference/ddl/create/#watermark>), and a [distribution](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/reference/ddl/create/#distributed>) to an existing materialized table.
...
Use `MODIFY` clause to change column’s comment, position, type (only non persisted like computed and metadata virtual also see [columns](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/reference/ddl/create/#columns>)), change primary key columns and watermark strategy to an existing table.
...
Use the `DROP` clause to drop columns (only non persisted like computed and metadata virtual, also see [columns](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/reference/ddl/create/#columns>)), primary key, partitions, and watermark strategy to an existing table.
...
Note
  ...
  * When suspending a table in CONTINUOUS mode, the job will be paused using STOP WITH SAVEPOINT by default. You need to set the SAVEPOINT save path using [parameters](<//nightlies.apache.org/flink/flink-docs-master/docs/deployment/config/#execution-checkpointing-savepoint-dir>).
...
Note When run through the Flink SQL Gateway, the behavior depends on the refresh mode and current refresh status:
...
**Full mode:**
  ...
  2. The table is refreshed using the new query definition when the next refresh job is triggered:
     * If it is a partitioned table and [partition.fields.#.date-formatter](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/config/#partition-fields-date-formatter>) is correctly set, only the latest partition will be refreshed.
...
**Continuous mode:**
  ...
  3. Start a new refresh job to refresh the materialized table:
     ...
     * The starting offset of the data source is determined by the connector’s default implementation or the [dynamic hint](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/reference/queries/hints/#dynamic-table-options>) specified in the query.
