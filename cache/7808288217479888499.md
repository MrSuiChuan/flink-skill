...
If successfully started, you can access the Flink Web UI at <http://localhost:8081/>.
...
[flink-cdc-3.6.0-bin.tar.gz](<https://www.apache.org/dyn/closer.lua/flink/flink-cdc-3.6.0/flink-cdc-3.6.0-bin.tar.gz>) flink-cdc-3.6.0 directory will contain four directories: `bin`, `lib`, `log`, and `conf`.
...
**Download links are available only for stable releases, SNAPSHOT dependencies need to be built based on master or release branches by yourself.** **Please note that you need to move the jar to the lib directory of Flink CDC Home, not to the lib directory of Flink Home.**
  ...
  3. Write task configuration yaml file. Here is an example file for synchronizing the entire database `postgres-to-fluss.yaml`：
