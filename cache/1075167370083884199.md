...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/python/table/python_types/>).
...
可以在[下面](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/types/#list-of-data-types>)找到所有预定义数据类型的列表。

##  数据类型（Data Type）和Python类型的映射关系 #

_数据类型_ 可用于声明Python用户自定义函数的输入/输出类型。输入数据将被转换为与所定义的数据类型相对应的Python对象，用户自定义函数的执行结果的类型也必须与所定义的数据类型匹配。
