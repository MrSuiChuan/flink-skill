...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/deployment/finegrained_resource/>).
...
> **Note:** This feature is currently an MVP (“minimum viable product”) feature and only available to [DataStream API](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/dev/datastream/overview/>).
...
As described in [Flink Architecture](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/concepts/flink-architecture/#anatomy-of-a-flink-cluster>), task execution resources in a TaskManager are split into many slots. The slot is the basic unit of both resource scheduling and resource requirement in Flink’s runtime.

With fine-grained resource management, the slots requests contain specific resource profiles, which users can specify. Flink will respect those user-specified resource requirements and dynamically cut an exactly-matched slot out of the TaskManager’s available resources. As shown above, there is a requirement for a slot with 0.25 Core and 1GB memory, and Flink allocates _Slot 1_ for it.
...
For the resource requirement without a specified resource profile, Flink will automatically decide a resource profile. Currently, the resource profile of it is calculated from [TaskManager’s total resource](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/memory/mem_setup_tm/>) and [taskmanager.numberOfTaskSlots](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/config/#taskmanager-numberoftaskslots>), just like in coarse-grained resource management. As shown above, the total resource of TaskManager is 1 Core and 4 GB memory and the number of task slots is set to 2, _Slot 2_ is created with 0.5 Core and 2 GB memory for the requirement without a specified resource profile.

After the allocation of _Slot 1_ and _Slot 2_ , there is 0.25 Core and 1 GB memory remaining as the free resources in the TaskManager. These free resources can be further partitioned to fulfill the following resource requirements.
...
To enable fine-grained resource management, you need to configure the [cluster.fine-grained-resource-management.enabled](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/config/#cluster-fine-grained-resource-management-enabled>) to true.
...
There are two approaches to define the slot sharing group and the operators it contains:
  ...
  * You can define a slot sharing group only by its name and attach it to an operator through the [slotSharingGroup(String name)](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/dev/datastream/operators/overview/#set-slot-sharing-group>).
...
You can specify the resource profile for your slot sharing groups:
  ...
  * If you only set the name of slot sharing group with [slotSharingGroup(String name)](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/dev/datastream/operators/overview/#set-slot-sharing-group>). You can construct a SlotSharingGroup instance with the same name along with the resource profile and register the resource of them with `StreamExecutionEnvironment#registerSlotSharingGroup(SlotSharingGroup ssg)`.
...
In constructing the `SlotSharingGroup`, you can set the following resource components for the slot sharing group:
  ...
  * **[Task Heap Memory](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/memory/mem_setup_tm/#task-operator-heap-memory>)**. Defines how much task heap memory is needed. Required to be explicitly configured with positive value.
  * **[Task Off-Heap Memory](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/memory/mem_setup_tm/#configure-off-heap-memory-direct-or-native>)**. Defines how much task off-heap memory is needed, can be 0.
  * **[Managed Memory](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/memory/mem_setup_tm/#managed-memory>)**. Defines how much task managed memory is needed, can be 0.
  * **[External Resources](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/advanced/external_resources/>)**. Defines the external resources needed, can be empty.
...
Since fine-grained resource management is a new, experimental feature, not all features supported by the default scheduler are also available with it. The Flink community is working on addressing these limitations.
  ...
  * **No support for the[Elastic Scaling](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/elastic_scaling/>)**. The elastic scaling only supports slot requests without specified-resource at the moment.

  * **No support for task manager redundancy**. The [slotmanager.redundant-taskmanager-num](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/config/#slotmanager-redundant-taskmanager-num>) is used to start redundant TaskManagers to speed up job recovery. This config option will not take effect in fine-grained resource management at the moment.

  * **No support for evenly spread out slot strategy**. This strategy tries to spread out the slots evenly across all available TaskManagers. The strategy is not supported in the first version of fine-grained resource management and [cluster.evenly-spread-out-slots](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/config/#cluster-evenly-spread-out-slots>) will not take effect in it at the moment.
...
##  Notice #
  ...
  * **Setting the slot sharing group may change the performance**. Setting chain-able operators to different slot sharing groups may break [operator chains](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/dev/datastream/operators/overview/#task-chaining-and-resource-groups>), and thus change the performance.
...
In this section, we talk about the slot partitioning mechanism in Flink runtime and the resource allocation strategy, including how the Flink runtime selects a TaskManager to cut slots and allocates TaskManagers on [Native Kubernetes](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/resource-providers/native_kubernetes/>) and [YARN](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/resource-providers/yarn/>). Note that the resource allocation strategy is pluggable in Flink runtime and here we introduce its default implementation in the first step of fine-grained resource management. In the future, there might be various strategies that users can select for different scenarios.
...
In the current resource allocation strategy, Flink will traverse all the registered TaskManagers and select the first one who has enough free resources to fulfill the slot request. When there is no TaskManager that has enough free resources, Flink will try to allocate a new TaskManager when deploying on [Native Kubernetes](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/resource-providers/native_kubernetes/>) or [YARN](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/resource-providers/yarn/>). In the current strategy, Flink will allocate identical TaskManagers according to [user’s configuration](<//nightlies.apache.org/flink/flink-docs-release-1.16/docs/deployment/memory/mem_setup_tm/#set-up-taskmanager-memory>). As the resource spec of TaskManagers is pre-defined:
