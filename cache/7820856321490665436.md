...
Flink supports developing relational applications in pure [SQL](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/table/sql/overview/>) or using the [Table API](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/table/tableapi/>). The Table API is a fluent DSL inspired by SQL, that can be written in Java or Python and supports strong IDE integration. Just like a SQL query, Table programs can select the required fields and group by your keys. These features, along with [built-in functions](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/table/functions/systemfunctions/>) like `floor` and `sum`, enable you to write this report.
...
Flink contains a limited number of built-in functions, and sometimes you need to extend it with a [user-defined function](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/table/functions/udfs/>). If `floor` wasn’t predefined, you could implement it yourself.
