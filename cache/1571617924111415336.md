...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/dev/datastream/fault-tolerance/serialization/custom_serialization/>).
...
####  Off-heap state backends (e.g. `EmbeddedRocksDBStateBackend`) #
  ...
  * Upon receiving the new serializer, the previous serializer’s snapshot is provided to the new serializer’s snapshot via the `TypeSerializer#resolveSchemaCompatibility` to check for schema compatibility.
