...
This connector provides access to partitioned files in filesystems supported by the [Flink FileSystem abstraction](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/deployment/filesystems/overview/>).
...
> Make sure to include [Flink File System specific dependencies](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/deployment/filesystems/overview/>).
...
The file system table supports both partition inserting and overwrite inserting. See [INSERT Statement](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/dev/table/sql/insert/>). When you insert overwrite to a partitioned table, only the corresponding partition will be overwritten, not the entire table.
...
The file system connector supports multiple formats:
  ...
  * Debezium-JSON: [debezium-json](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/connectors/table/formats/debezium/>).
  * Canal-JSON: [canal-json](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/connectors/table/formats/canal/>).
  * Raw: [raw](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/connectors/table/formats/raw/>).
...
The file system connector supports streaming writes, based on Flink’s [FileSystem](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/connectors/datastream/filesystem/>), to write records to file. Row-encoded Formats are CSV and JSON. Bulk-encoded Formats are Parquet, ORC and Avro.

You can write SQL directly, insert the stream data into the non-partitioned table. If it is a partitioned table, you can configure partition related operations. See [Partition Commit](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/connectors/table/filesystem/#partition-commit>) for details.
...
| (none) | Integer | Parallelism of writing files into external file system. The value should greater than zero otherwise exception will be thrown.
