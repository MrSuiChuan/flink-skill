...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/table/concepts/versioned_tables/>).

#  时态表（Temporal Tables） #

时态表（Temporal Table）是一张随时间变化的表 – 在 Flink 中称为[动态表](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/concepts/dynamic_tables/>)，时态表中的每条记录都关联了一个或多个时间段，所有的 Flink 表都是时态的（动态的）。
...
为了在 `RatesHistory` 上定义版本表，Flink 支持通过[去重查询](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/deduplication/#%e5%8e%bb%e9%87%8d>)定义版本视图， 去重查询可以产出一个有序的 changelog 流，去重查询能够推断主键并保留原始数据流的事件时间属性。
...
普通表的声明和 Flink 建表 DDL 一致，参考 [create table](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/create/#create-table>) 页面获取更多如何建表的信息。
...
通过 `LookupableTableSource` 定义的表意味着该表具备了在运行时通过一个或多个 key 去查询外部存储系统的能力，当前支持在 基于处理时间的时态表 join 中使用的表包括 [JDBC](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/connectors/table/jdbc/>), [HBase](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/connectors/table/hbase/>) 和 [Hive](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/connectors/table/hive/hive_read_write/#temporal-table-join>)。

另请参阅 [LookupableTableSource](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sourcessinks/#lookup-table-source>)页面了解更多信息。
