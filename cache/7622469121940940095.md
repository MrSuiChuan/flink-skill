...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/connectors/table/datagen/>).

#  DataGen SQL 连接器 #

Scan Source: 有界 Scan Source: 无界

DataGen 连接器允许基于内存生成数据来创建表。 在本地开发时，若不访问外部系统（如 Kafka），这会非常有用。 可以使用[计算列语法](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/ddl/create/#create-table>)灵活地生成记录。
