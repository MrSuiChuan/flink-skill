...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/table/sql/queries/window-agg/>).
...
窗口聚合是通过 `GROUP BY` 子句定义的，其特征是包含 [窗口表值函数](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/window-tvf/>) 产生的 “window_start” 和 “window_end” 列。和普通的 `GROUP BY` 子句一样，窗口聚合对于每个组会计算出一行数据。
...
Flink 支持在 `TUMBLE`，`HOP`，`CUMULATE` 和 `SESSION` 上进行窗口聚合。 在流模式下，窗口表值函数的时间属性字段必须是 [事件时间或处理时间](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/concepts/time_attributes/>)。关于窗口函数更多信息，参见 [Windowing TVF](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/window-tvf/>)。 在批模式下，窗口表值函数的时间属性字段必须是 `TIMESTAMP` 或 `TIMESTAMP_LTZ` 类型的。
...
_注意: 为了更好地理解窗口行为，这里把 timestamp 值后面的 0 去掉了，例如：在 Flink SQL Client 中，如果类型是`TIMESTAMP(3)`，`2020-04-15 08:05` 应该显示成 `2020-04-15 08:05:00.000`。_
...
`window_start` 和 `window_end` 列是普通的时间戳字段，并不是时间属性。因此它们不能在后续的操作中当做时间属性进行基于时间的操作。 为了传递时间属性，需要在 `GROUP BY` 子句中添加 `window_time` 列。`window_time` 是 [Windowing TVFs](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/window-tvf/#window-functions>) 产生的三列之一，它是窗口的时间属性。 `window_time` 添加到 `GROUP BY` 子句后就能被选定了。下面的查询可以把它用于后续基于时间的操作，比如：多级窗口聚合 和 [Window TopN](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/window-topn/>)。
...
>
>   * 包含 [性能调优](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/tuning/>) 中提到的所有性能优化。
>   * 支持标准的 `GROUPING SETS` 语法。
>   * 可以在窗口聚合结果上使用 [窗口 TopN](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/window-topn/>)。
>   * 等等。
>
...
在流处理模式，分组窗口函数的 `time_attr` 属性必须是一个有效的处理或事件时间。更多关于定义时间属性参见：[时间属性文档](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/concepts/time_attributes/>)
...
| 返回相应滚动窗口，跳跃窗口或会话窗口的上限的时间戳（exclusive），即窗口结束时间。 **注意:** 上限时间戳（exlusive） _不能_ 作为 [rowtime attribute](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/concepts/time_attributes/>) 用于后续基于时间的操作，例如：[interval joins](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/joins/#interval-joins>) 和 [group window](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/window-agg/>) 或 [over window aggregations](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/over-agg/>)。
`TUMBLE_ROWTIME(time_attr, interval)`
`HOP_ROWTIME(time_attr, interval, interval)`
`SESSION_ROWTIME(time_attr, interval)`
| 返回相应滚动窗口，跳跃窗口或会话窗口的上限的时间戳（inclusive），即窗口事件时间，或窗口处理时间。 返回的值是 [rowtime attribute](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/concepts/time_attributes/>)，可以用于后续基于时间的操作，比如：[interval joins](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/joins/#interval-joins>) 和 [group window](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/window-agg/>) 或 [over window aggregations](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/over-agg/>)。
`TUMBLE_PROCTIME(time_attr, interval)`
`HOP_PROCTIME(time_attr, interval, interval)`
`SESSION_PROCTIME(time_attr, interval)`
| 返回的值是 [proctime attribute](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/concepts/time_attributes/#processing-time>)，可以用于后续基于时间的操作，比如： [interval joins](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/joins/#interval-joins>) 和 [group window](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/window-agg/>) 或 [over window aggregations](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/queries/over-agg/>)。

_注意：_ 辅助函数的参数必须和 `GROUP BY` 子句中的分组窗口函数一致。
