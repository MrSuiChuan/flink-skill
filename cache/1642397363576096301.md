...
Apache Flink ships with a universal Kafka connector which attempts to track the latest version of the Kafka client. The version of the client it uses may change between Flink releases. Modern Kafka clients are backwards compatible with broker versions 0.10.0 or later. For details on Kafka compatibility, please refer to the official [Kafka documentation](<https://kafka.apache.org/protocol.html#protocol_compatibility>).
    ...
    &ltdependency&gt
        ...
        &ltartifactId&gtflink-connector-kafka_2.11&lt/artifactId&gt
...
if you are using Kafka source, `flink-connector-base` is also required as dependency:
    ...
    &ltdependency&gt
        &ltgroupId&gtorg.apache.flink&lt/groupId&gt
        &ltartifactId&gtflink-connector-base&lt/artifactId&gt
        &ltversion&gt1.13.6&lt/version&gt
    &lt/dependency&gt

Copied to clipboard!

Flink’s streaming connectors are not currently part of the binary distribution. See how to link with them for cluster execution [here](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/project-configuration/>).

##  Kafka Source #

> This part describes the Kafka source based on the new [data source](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/sources/>) API.
...
[This documentation](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/event-time/generating_watermarks/>) describes details about how to define a `WatermarkStrategy`.
...
[This documentation](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/event-time/generating_watermarks/#dealing-with-idle-sources>) describes details about how to define a `WatermarkStrategy#withIdleness`.
...
> If you are interested in how Kafka source works under the design of new data source API, you may want to read this part as a reference. For details about the new data source API, [documentation of data source](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/sources/>) and [FLIP-27](<https://cwiki.apache.org/confluence/display/FLINK/FLIP-27%3A+Refactor+Source+Interface>) provide more descriptive discussions.
...
In many scenarios, the timestamp of a record is embedded in the record itself, or the metadata of the `ConsumerRecord`. In addition, users may want to emit watermarks either periodically, or irregularly, e.g. based on special records in the Kafka stream that contain the current event-time watermark. For these cases, the Flink Kafka Consumer allows the specification of a [watermark strategy](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/concepts/time/>).

You can specify your custom strategy as described [here](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/event-time/generating_watermarks/>), or use one from the [predefined ones](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/event-time/built_in/>).
...
Flink’s Kafka connectors provide some metrics through Flink’s [metrics system](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/ops/metrics/>) to analyze the behavior of the connector. The producers export Kafka’s internal metrics through Flink’s metric system for all supported versions. The Kafka documentation lists all exported metrics in its [documentation](<http://kafka.apache.org/documentation/#selector_monitoring>).
...
For more information on Flink configuration for Kerberos security, please see [here](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/deployment/config/>). You can also find [here](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/deployment/security/security-kerberos/>) further details on how Flink internally setups Kerberos-based security.

##  Upgrading to the Latest Connector Version #

The generic upgrade steps are outlined in [upgrading jobs and Flink versions guide](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/ops/upgrading/>). For Kafka, you additionally need to follow these steps:
