...
参考 [Checkpointing](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/datastream/fault-tolerance/checkpointing/>) 查看如何在 Flink 程序中开启和配置 checkpoint。

要了解 checkpoints 和 [savepoints](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/ops/state/savepoints/>) 之间的区别，请参阅 [checkpoints 与 savepoints](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/ops/state/checkpoints_vs_savepoints/>)。
...
`JobManagerCheckpointStorage` 的限制:
  ...
  * 无论配置的最大 State 大小如何，状态都不能大于 Pekko 框架的大小（请参阅 [配置参数](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/config/>)）。
...
与 [savepoints](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/ops/state/savepoints/>) 相似，checkpoint 由元数据文件、数据文件（与 state backend 相关）组成。可通过配置文件中 “execution.checkpointing.dir” 配置项来指定元数据文件和数据文件的存储路径，另外也可以在代码中针对单个作业特别指定该配置项。
...
与 savepoint 一样，作业可以从 checkpoint 的元数据文件恢复运行（[savepoint恢复指南](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/ops/state/savepoints/#resuming-from-savepoints>)）。注意，如果元数据文件中信息不充分，那么 jobmanager 就需要使用相关的数据文件来恢复作业(参考目录结构)。
