...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/learn-flink/streaming_analytics/>).
...
如果想要使用事件时间，需要额外给 Flink 提供一个时间戳提取器和 Watermark 生成器，Flink 将使用它们来跟踪事件时间的进度。这将在选节[使用 Watermarks](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/learn-flink/streaming_analytics/#working-with-watermarks>) 中介绍，但是首先我们需要解释一下 watermarks 是什么。
...
我们可能在有些场景下，想使用全局 window assigner 将每个事件（相同的 key）都分配给某一个指定的全局窗口。 很多情况下，一个比较好的建议是使用 `ProcessFunction`，具体介绍在[这里](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/learn-flink/event_driven/#process-functions>)。
...
您可以使用一种称为[旁路输出](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/learn-flink/event_driven/#side-outputs>) 的机制来安排将要删除的事件收集到侧输出流中，这里是一个示例:
...
请注意，滑动窗口和滚动窗口分配器所采用的 offset 参数可用于改变窗口的对齐方式。有关详细的信息，请参见 [滚动窗口](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/datastream/operators/windows/#tumbling-windows>) 和 [滑动窗口](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/datastream/operators/windows/#sliding-windows>) 。
...
本节附带的动手练习是 [ Hourly Tips Exercise ](<//github.com/apache/flink-training/blob/master//hourly-tips>) .

__Back to top

##  延伸阅读 #

  * [Timely Stream Processing](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/concepts/time/>)
  * [Windows](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/datastream/operators/windows/>)
