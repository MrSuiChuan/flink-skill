...
Avro schemas are defined using JSON. You can get more information about Avro schemas and types from the [Avro specification](<https://avro.apache.org/docs/++version++/spec.html>). This example uses an Avro schema example similar to the one described in the [official Avro tutorial](<https://avro.apache.org/docs/++version++/getting-started-java>):
...
The following example uses the example schema [ testdata.avsc ](<//github.com/apache/flink/blob/release-1.20/flink-formats/flink-parquet/src/test/resources/avro/testdata.avsc>) :
...
Beyond Avro Generic and Specific record that requires a predefined Avro schema, Flink also supports creating a DataStream from Parquet files based on existing Java POJO classes. In this case, Avro will use Java reflection to generate schemas and protocols for these POJO classes. Java types are mapped to Avro schemas, please refer to the [Avro reflect](<https://avro.apache.org/docs/++version++/api/java/index.html>) documentation for more details.

This example uses a simple Java POJO class [ Datum ](<//github.com/apache/flink/blob/release-1.20/flink-formats/flink-parquet/src/test/java/org/apache/flink/formats/parquet/avro/Datum.java>) :
