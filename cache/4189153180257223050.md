...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/deployment/ha/overview/>).
...
With JobManager High Availability, you can recover from JobManager failures and thereby eliminate the _SPOF_. You can configure high availability for every cluster deployment. See the list of available high availability services for more information.

###  How to make a cluster highly available #

The general idea of JobManager High Availability is that there is a _single leading JobManager_ at any time and _multiple standby JobManagers_ to take over leadership in case the leader fails. This guarantees that there is _no single point of failure_ and programs can make progress as soon as a standby JobManager has taken leadership.
...
Flink ships with two high availability service implementations:
  ...
  * [ZooKeeper](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/deployment/ha/zookeeper_ha/>): ZooKeeper HA services can be used with every Flink cluster deployment. They require a running ZooKeeper quorum.

  * [Kubernetes](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/deployment/ha/kubernetes_ha/>): Kubernetes HA services only work when running on Kubernetes.
...
The JobResultStore is used to archive the final result of a job that reached a globally-terminal state (i.e. finished, cancelled or failed). The data is stored on a file system (see [job-result-store.storage-path](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/deployment/config/#job-result-store-storage-path>)). Entries in this store are marked as dirty as long as the corresponding job wasn’t cleaned up properly (artifacts are found in the job’s subfolder in [high-availability.storageDir](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/deployment/config/#high-availability-storagedir>)).

Dirty entries are subject to cleanup, i.e. the corresponding job is either cleaned up by Flink at the moment or will be picked up for cleanup as part of a recovery. The entries will be deleted as soon as the cleanup succeeds. Check the JobResultStore configuration parameters under [HA configuration options](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/deployment/config/#high-availability>) for further details on how to adapt the behavior.
