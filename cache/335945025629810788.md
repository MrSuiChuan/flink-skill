...
Different operator metrics can be turned on/off individually using the configuration. For details check the [metrics config reference](<//nightlies.apache.org/flink/flink-kubernetes-operator-docs-release-1.15/docs/operations/configuration/#system-metrics-configuration>).

###  Scope #

Every metric emitted by the operator is published under a metric group. The operator defines three scopes, each with a configurable scope format (see the [Configuration](<//nightlies.apache.org/flink/flink-kubernetes-operator-docs-release-1.15/docs/operations/configuration/>) page for the full list of scope variables):
...
Based on the resource status the operator monitors [resource lifecycle states](<//nightlies.apache.org/flink/flink-kubernetes-operator-docs-release-1.15/docs/concepts/architecture/#flink-resource-lifecycle>) for `FlinkDeployment` and `FlinkSessionJob` resources.
...
Reporter options follow Flink’s schema verbatim and are therefore not repeated on the [Configuration](<//nightlies.apache.org/flink/flink-kubernetes-operator-docs-release-1.15/docs/operations/configuration/>) page. Only operator-specific toggles (JVM / JOSDK / Kubernetes client metrics, resource & lifecycle metrics, scope formats) are listed there.
