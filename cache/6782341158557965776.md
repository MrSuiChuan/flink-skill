...
To make proper use of savepoints, it’s important to understand the differences between [checkpoints](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/ops/state/checkpoints/>) and savepoints which is described in [checkpoints vs. savepoints](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/ops/state/checkpoints_vs_savepoints/>).
...
You can use the [command line client](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/deployment/cli/#savepoints>) to _trigger savepoints_ , _cancel a job with a savepoint_ , _resume from savepoints_ , and _dispose savepoints_.
...
>
>   1. if [_entropy injection_](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/deployment/filesystems/s3/#entropy-injection-for-s3-file-systems>) is activated: In that case the savepoint directory will not contain all savepoint data files, because the injected path entropy spreads the files over many directories. Lacking a common savepoint root directory, the savepoints will contain absolute path references, which prevent moving the directory.
>
...
When using this command, the client returns immediately after getting the trigger id of the savepoint. You can monitor the status of the savepoint through the REST API [rest api](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/ops/rest_api//#jobs-jobid-checkpoints-triggerid>).
...
Each claim mode serves a specific purposes. Still, we believe the default _NO_CLAIM_ mode is a good tradeoff in most situations, as it provides clear ownership with a small price for the first checkpoint after the restore.
...
>
>   2. Native format supports incremental RocksDB savepoints. For those savepoints Flink puts all SST files inside the savepoints directory. This means such savepoints are self-contained and relocatable. Please note that, when restored in CLAIM mode, subsequent checkpoints might reuse some SST files, which might delay the deletion the savepoints directory.
>
>
