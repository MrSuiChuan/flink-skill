...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/deployment/security/security-ssl/>).
...
This example shows how to create a simple keystore / truststore pair. The truststore does not contain the primary key and can be shared with other applications. In this example, _myhost.company.org / ip:10.0.2.15_ is the node (or service) for the JobManager.
...
Now create a keystore for the REST endpoint with a certificate signed by the above CA. Let _flink.company.org / ip:10.0.2.15_ be the hostname of the JobManager.
...
Now add the following configuration to your [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-master/docs/deployment/config/#flink-configuration-file>):
