...
The Elasticsearch connector is not part of the binary distribution. See how to link with it for cluster execution [here](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/configuration/overview/>).
...
| required in 6.x | yes in 6.x | (none) | String | Elasticsearch document type. Not necessary anymore in `elasticsearch-7`.
...
| optional | no | json | String | Elasticsearch connector supports to specify a format. The format must produce a valid json document. By default uses built-in `'json'` format. Please refer to [JSON Format](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/connectors/table/formats/overview/>) page for more details.
...
See [CREATE TABLE DDL](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/table/sql/create/#create-table>) for more details about the PRIMARY KEY syntax.
...
Elasticsearch stores document in a JSON string. So the data type mapping is between Flink data type and JSON data type. Flink uses built-in `'json'` format for Elasticsearch connector. Please refer to [JSON Format](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/connectors/table/formats/json/>) page for more type mapping details.
