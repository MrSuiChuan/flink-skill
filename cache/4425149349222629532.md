...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/tableapi/>).
...
The Table API shares many concepts and parts of its API with Flink’s SQL integration. Have a look at the [Common Concepts & API](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/common/>) to learn how to register tables or to create a `Table` object. The [Streaming Concepts](<//nightlies.apache.org/flink/flink-docs-master/docs/concepts/sql-table-concepts/overview/>) pages discuss streaming specific concepts such as dynamic tables and time attributes.

The following examples assume a registered table called `Orders` with attributes `(a, b, c, rowtime)`. The `rowtime` field is either a logical [time attribute](<//nightlies.apache.org/flink/flink-docs-master/docs/concepts/sql-table-concepts/time_attributes/>) in streaming or a regular timestamp field in batch.
...
Performs a map operation with a python [general scalar function](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/python-udfs/#scalar-functions>) or [vectorized scalar function](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/python-udfs/#vectorized-scalar-functions>). The output will be flattened if the output type is a composite type.
...
Performs a `flat_map` operation with a python [table function](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/python-udfs/#table-functions>).
...
Performs an aggregate operation with a python [general aggregate function](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/python-udfs/#aggregate-functions>) or [vectorized aggregate function](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/python-udfs/#vectorized-aggregate-functions>). You have to close the “aggregate” with a select statement and the select statement does not support aggregate functions. The output of aggregate will be flattened if the output type is a composite type.
...
Performs a flat_aggregate operation with a python general [Table Aggregate Function](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/python-udfs/#table-aggregate-functions>)
...
Please see the dedicated page about [data types](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/reference/data-types/>).

Generic types and (nested) composite types (e.g., POJOs, tuples, rows, Scala case classes) can be fields of a row as well.

Fields of composite types with arbitrary nesting can be accessed with [value access functions](<//nightlies.apache.org/flink/flink-docs-master/docs/sql/functions/built-in-functions/#value-access-functions>).

Generic types are treated as a black box and can be passed on or processed by [user-defined functions](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/udfs/>).
