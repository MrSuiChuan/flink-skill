...
In `STREAMING` mode, Flink uses a [StateBackend](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/fault-tolerance/state_backends/>) to control how state is stored and how checkpointing works.
...
There are three general types of input that we can differentiate:
  ...
  * _broadcast input_ : input from a broadcast stream (see also [Broadcast State](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/fault-tolerance/broadcast_state/>))
...
In `STREAMING` execution mode, Flink uses checkpoints for failure recovery. Take a look at the [checkpointing documentation](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/fault-tolerance/checkpointing/>) for hands-on documentation about this and how to configure it. There is also a more introductory section about [fault tolerance via state snapshots](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/learn-flink/fault_tolerance/>) that explains the concepts at a higher level.
...
Behavior Change in BATCH mode:
  ...
  * “Rolling” operations such as [reduce()](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/operators/overview/#reduce>) or sum() emit an incremental update for every new record that arrives in `STREAMING` mode. In `BATCH` mode, these operations are not “rolling”. They emit only the final result.

Unsupported in BATCH mode:

  * [Checkpointing](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/concepts/stateful-stream-processing/#checkpointing>) and any operations that depend on checkpointing do not work.
...
It is important to remember that because there are no checkpoints, certain features such as [ CheckpointListener ](<//nightlies.apache.org/flink/flink-docs-release-2.0/api/java//org/apache/flink/api/common/state/CheckpointListener.html>) and, as a result, Kafka’s [EXACTLY_ONCE](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/connectors/datastream/kafka/#kafka-producers-and-fault-tolerance>) mode or `File Sink`’s [OnCheckpointRollingPolicy](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/connectors/datastream/filesystem/#rolling-policy>) won’t work. If you need a transactional sink that works in `BATCH` mode make sure it uses the Unified Sink API as proposed in [FLIP-143](<https://cwiki.apache.org/confluence/x/KEJ4CQ>).

You can still use all the [state primitives](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/fault-tolerance/state/#working-with-state>), it’s just that the mechanism used for failure recovery will be different.
