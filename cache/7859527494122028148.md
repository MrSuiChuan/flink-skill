...
Below, we briefly explain the building blocks of a Flink cluster, their purpose and available implementations. If you just want to start Flink locally, we recommend setting up a [Standalone Cluster](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/resource-providers/standalone/overview/>).
...
Flink Client |  Compiles batch or streaming applications into a dataflow graph, which it then submits to the JobManager.  |
  ...
  * [Command Line Interface](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/cli/>)
  * [REST Endpoint](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/ops/rest_api/>)
  * [SQL Client](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/sql/interfaces/sql-client/>)
  * [Python REPL](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/repls/python_shell/>)
...
|
  ...
  * [Standalone](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/resource-providers/standalone/overview/>) (this is the barebone mode that requires just JVMs to be launched. Deployment with [Docker, Docker Swarm / Compose](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/resource-providers/standalone/docker/>), [non-native Kubernetes](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/resource-providers/standalone/kubernetes/>) and other models is possible through manual setup in this mode)
  * [Kubernetes](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/resource-providers/native_kubernetes/>)
  * [YARN](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/resource-providers/yarn/>)
...
High Availability Service Provider |  Flink's JobManager can be run in high availability mode which allows Flink to recover from JobManager faults. In order to failover faster, multiple standby JobManagers can be started to act as backups.  |
  ...
  * [Zookeeper](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/ha/zookeeper_ha/>)
  * [Kubernetes HA](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/ha/kubernetes_ha/>)


File Storage and Persistency |  For checkpointing (recovery mechanism for streaming jobs) Flink relies on external file storage systems  | See [FileSystems](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/filesystems/overview/>) page.
...
Metrics Storage |  Flink components report internal metrics and Flink jobs can report additional, job specific metrics as well.  | See [Metrics Reporter](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/metric_reporters/>) page.
...
See [Connectors](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/connectors/datastream/overview/>) page.

###  Repeatable Resource Cleanup #

Once a job has reached a globally terminal state of either finished, failed or cancelled, the external component resources associated with the job are then cleaned up. In the event of a failure when cleaning up a resource, Flink will attempt to retry the cleanup. You can [configure](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/config/#retryable-cleanup>) the retry strategy used. Reaching the maximum number of retries without succeeding will leave the job in a dirty state. Its artifacts would need to be cleaned up manually (see the [High Availability Services / JobResultStore](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/ha/overview/#jobresultstore>) section for further details). Restarting the very same job (i.e. using the same job ID) will result in the cleanup being restarted without running the job again.
...
The application resource cleanup is similar (see the [High Availability Services / ApplicationResultStore](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/ha/overview/#applicationresultstore>) section for further details).
