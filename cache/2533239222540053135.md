...
When the streaming application works with [event time](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/concepts/time/>), watermarks will be handled correctly by the asynchronous I/O operator. That means concretely the following for the two order modes:
