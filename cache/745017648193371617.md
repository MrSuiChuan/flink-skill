...
This strategy is enabled as default by setting the following configuration parameter in [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/deployment/config/#flink-configuration-file>).
...
Flink supports different failover strategies which can be configured via the configuration parameter _jobmanager.execution.failover-strategy_ in [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/deployment/config/#flink-configuration-file>).
...
DataStream/Table/SQL job data exchanges are determined by the `ExecutionMode`, which can be set through [ExecutionConfig](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/dev/datastream/execution/execution_configuration/>), which are pipelined in Streaming Mode, are batched by default in Batch Mode.
