...
Every time a message is sent a greeter instance, it is interpreted as a `string` representing the users name. Both messages and state are strongly typed - either one of the default [built-in types](<//nightlies.apache.org/flink/flink-statefun-docs-release-3.1/docs/sdk/appendix/#types>) \- or a custom type.
