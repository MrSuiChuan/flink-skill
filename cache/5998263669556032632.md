...
Flink 提供了 Maven Archetype 来快速创建包含所有必要依赖的项目骨架，这样你只需要专注于填写业务逻辑。 这些依赖包括 `flink-streaming-java`（所有 Flink 流处理应用的核心依赖）和 `flink-walkthrough-common`（包含本教程所需的数据生成器和其他类）。
...
为此，欺诈检测器必须 _记住_ 跨事件的信息；只有当上一笔交易是小额交易时，大额交易才是欺诈交易。跨事件记住信息需要使用 [状态](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/concepts/glossary/#managed-state>)，这就是本教程使用 [KeyedProcessFunction](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/datastream/operators/process_function/>) 的原因。它提供了对状态和时间的细粒度控制，这将允许你使用更复杂的需求来演进算法。
...
Flink 中最基本的状态类型是 [ValueState](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/datastream/fault-tolerance/state/>)，一种为其包装的任何变量添加容错能力的数据类型。`ValueState` 是一种 _keyed state_ ，这意味着它只能在 _keyed context_ 中的算子中使用；任何紧跟在 `keyBy` 之后调用的算子。算子的 _keyed state_ 自动限定在当前处理记录的 key 范围内。在这个例子中，key 是当前交易的账户 ID（由 `keyBy()` 声明），`FraudDetector` 为每个账户维护独立的状态。
...
上述命令会构建 PyFlink 程序，并在本地 mini cluster 中运行。如果想将作业提交到远端集群执行，可以参考[作业提交示例](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/cli/#submitting-pyflink-jobs>)。
...
要了解如何为欺诈检测器添加定时器（例如，只标记在 1 分钟内发生的交易），请参阅 Learn Flink 中的[事件驱动应用](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/learn-flink/event_driven/>)部分。

###  深入了解 DataStream #

  * [DataStream API 概述](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/datastream/overview/>)：完整的 DataStream API 参考
  * [处理函数](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/datastream/operators/process_function/>)：深入了解 KeyedProcessFunction 和其他处理函数
  * [状态与容错](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/datastream/fault-tolerance/state/>)：理解托管状态和检查点

###  探索其他教程 #

  * [Flink SQL 教程](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/getting-started/quickstart-sql/>)：无需编码的交互式 SQL 查询
  * [Table API 教程](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/getting-started/table_api/>)：使用 Table API 构建流处理管道
...
###  生产部署 #
  ...
  * [连接器](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/connectors/datastream/overview/>)：连接 Kafka、数据库、文件系统等