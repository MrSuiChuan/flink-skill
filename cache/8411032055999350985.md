...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/configuration/maven/>).
...
如果您想 **构建/打包** 您的项目，请转到您的项目目录并运行 ‘`mvn clean package`’ 命令。您将 **找到一个 JAR 文件** ，其中包含您的应用程序（还有已作为依赖项添加到应用程序的连接器和库）：`target/<artifact-id>-<version>.jar`。
