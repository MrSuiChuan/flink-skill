...
Apache Flink ships with a universal Kafka connector which attempts to track the latest version of the Kafka client. The version of the client it uses may change between Flink releases. Modern Kafka clients are backwards compatible with broker versions 0.10.0 or later. For details on Kafka compatibility, please refer to the official [Kafka documentation](<https://kafka.apache.org/protocol.html#protocol_compatibility>).
    ...
    &ltdependency&gt
        ...
        &ltversion&gt3.2.0-1.18&lt/version&gt
    &lt/dependency&gt
...
Version | PyFlink JAR | flink-connector-kafka | [Download](<https://repo.maven.apache.org/maven2/org/apache/flink/flink-sql-connector-kafka/3.2.0-1.18/flink-sql-connector-kafka-3.2.0-1.18.jar>)
---|---
...
> This part describes the Kafka source based on the new [data source](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/datastream/sources/>) API.
...
You can also use a [`Kafka Deserializer`](<https://kafka.apache.org/24/javadoc/org/apache/kafka/common/serialization/Deserializer.html>) for deserializing Kafka message value. For example using `StringDeserializer` for deserializing Kafka message value as string:
...
For configurations of KafkaConsumer, you can refer to [Apache Kafka documentation](<http://kafka.apache.org/documentation/#consumerconfigs>) for more details.
...
In order to handle scenarios like topic scaling-out or topic creation without restarting the Flink job, Kafka source can be configured to periodically discover new partitions under provided topic-partition subscribing pattern. To enable partition discovery, set a positive value for property `partition.discovery.interval.ms`:
...
[This documentation](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/datastream/event-time/generating_watermarks/>) describes details about how to define a `WatermarkStrategy`. (Not supported in PyFlink)
...
[This documentation](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/datastream/event-time/generating_watermarks/#dealing-with-idle-sources>) describes details about how to define a `WatermarkStrategy#withIdleness`.
...
Kafka source exposes the following metrics in the respective [scope](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/ops/metrics//#scope>).
...
> If you are interested in how Kafka source works under the design of new data source API, you may want to read this part as a reference. For details about the new data source API, [documentation of data source](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/datastream/sources/>) and [FLIP-27](<https://cwiki.apache.org/confluence/display/FLINK/FLIP-27%3A+Refactor+Source+Interface>) provide more descriptive discussions.
...
Kafka sink exposes the following metrics in the respective [scope](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/ops/metrics//#scope>).
...
Flink’s Kafka connectors provide some metrics through Flink’s [metrics system](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/ops/metrics/>) to analyze the behavior of the connector. The producers and consumers export Kafka’s internal metrics through Flink’s metric system for all supported versions. The Kafka documentation lists all exported metrics in its [documentation](<http://kafka.apache.org/documentation/#selector_monitoring>).
...
For more information on Flink configuration for Kerberos security, please see [here](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/config/>). You can also find [here](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/security/security-kerberos/>) further details on how Flink internally setups Kerberos-based security.

##  Upgrading to the Latest Connector Version #

The generic upgrade steps are outlined in [upgrading jobs and Flink versions guide](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/ops/upgrading/>). For Kafka, you additionally need to follow these steps:
