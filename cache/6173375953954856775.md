...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/concepts/sql-table-concepts/dynamic_tables/>).
...
第二条查询与第一条类似，但是除了用户属性之外，还将 `clicks` 分组至[每小时滚动窗口](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/sql/reference/overview/#group-windows>)中，然后计算 url 数量(基于时间的计算，例如基于特定[时间属性](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/concepts/sql-table-concepts/time_attributes/>)的窗口，后面会讨论)。同样，该图显示了不同时间点的输入和输出，以可视化动态表的变化特性。
...
[查询配置](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/config/>)章节讨论了控制连续查询执行的参数。一些参数可以用来在维持状态的大小和获得结果的准确性之间做取舍。
...
在[通用概念](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/common/#convert-a-table-into-a-datastream>)中讨论了将动态表转换为 `DataStream` 的 API。请注意，在将动态表转换为 `DataStream` 时，只支持 append 流和 retract 流。在 [TableSources 和 TableSinks](<//nightlies.apache.org/flink/flink-docs-master/zh/docs/dev/table/sourcessinks/#dynamic-table-sink>) 章节讨论向外部系统输出动态表的 `TableSink` 接口。
