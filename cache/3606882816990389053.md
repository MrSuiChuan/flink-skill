...
To implement a custom FailureEnricher plugin, you need to:
  ...
  * Add your own FailureEnricher by implementing the [ FailureEnricher ](<//github.com/apache/flink/blob/release-2.3//flink-core/src/main/java/org/apache/flink/core/failure/FailureEnricher.java>) interface.

  * Add your own FailureEnricherFactory by implementing the [ FailureEnricherFactory ](<//github.com/apache/flink/blob/release-2.3//flink-core/src/main/java/org/apache/flink/core/failure/FailureEnricherFactory.java>) interface.
...
Then, create a jar which includes your `FailureEnricher`, `FailureEnricherFactory`, `META-INF/services/` and all external dependencies. Make a directory in `plugins/` of your Flink distribution with an arbitrary name, e.g. “failure-enrichment”, and put the jar into this directory. See [Flink Plugin](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/filesystems/plugins/>) for more details.

> Note that every FailureEnricher should have defined a set of [ output keys ](<//github.com/apache/flink/blob/release-2.3//flink-core/src/main/java/org/apache/flink/core/failure/FailureEnricher.java>) that may be associated with values. This set of keys has to be unique otherwise all enrichers with overlapping keys will be ignored.
...
The JobManager loads FailureEnricher plugins at startup. To make sure your FailureEnrichers are loaded all class names should be defined as part of [jobmanager.failure-enrichers configuration](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/deployment/config/#jobmanager-failure-enrichers>). If this configuration is empty, NO enrichers will be started. Example:
...
Moreover, you can query the JobManager’s [REST API](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/ops/rest_api/#jobs-jobid-exceptions>) looking for the failureLabels field:
