...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/table/sql/analyze/>).
...
`ANALYZE TABLE` 语句可以在 [SQL CLI](<//nightlies.apache.org/flink/flink-docs-release-1.16/zh/docs/dev/table/sqlclient/>) 中执行。
...
_注意:_ 对于数据值定长的类型(例如：`BOOLEAN`, `INTEGER`, `DOUBLE` 等)， Flink 不会去收集 `avgLen` 和 `maxLen` 值。
