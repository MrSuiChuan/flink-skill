...
Note that these examples are _not_ exhaustive and you can use GCS in other places as well, including your [high availability setup](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/deployment/ha/overview/>) or the [EmbeddedRocksDBStateBackend](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/ops/state/state_backends/#the-rocksdbstatebackend>); everywhere that Flink expects a FileSystem URI.
...
This file system can be used with the [FileSystem connector](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/connectors/datastream/filesystem/>).
...
To use `flink-gs-fs-hadoop`, copy the JAR file from the `opt` directory to the `plugins` directory of your Flink distribution before starting Flink, i.e.
    ...
    cp ./opt/flink-gs-fs-hadoop-2.0.1.jar ./plugins/gs-fs-hadoop/


###  Configuration #

The underlying Hadoop file system can be configured using the [Hadoop configuration keys](<https://github.com/GoogleCloudDataproc/hadoop-connectors/blob/v2.2.18/gcs/CONFIGURATION.md>) for `gcs-connector` by adding the configurations to your [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/deployment/config/#flink-configuration-file>).

For example, `gcs-connector` has a `fs.gs.http.connect-timeout` configuration key. If you want to change it, you need to set `gs.http.connect-timeout: xyz` in [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/deployment/config/#flink-configuration-file>). Flink will internally translate this back to `fs.gs.http.connect-timeout`.
...
`flink-gs-fs-hadoop` can also be configured by setting the following options in [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/deployment/config/#flink-configuration-file>):
