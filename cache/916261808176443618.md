...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/table/procedures/>).
...
Table（类似于 SQL 标准）是一种强类型的 API。 因此，存储过程的参数和返回类型都必须映射到 [data type](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/types/>)。
...
有关可以隐式映射到数据类型的类的完整列表, 请参阅[data type extraction section](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/types/#data-type-extraction>)。
