...
A [Skill](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/development/skills/>) is a self-contained package of instructions, and optionally scripts and reference files, that teaches the agent how to perform a specialized task. Skills are loaded with _progressive disclosure_ : only each skill’s name and description are shown to the agent up front, and the full instructions are pulled in on demand when the agent decides a skill is relevant.
...
> See [Integrate with Flink](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/development/integrate_with_flink/>) for details on integrating agents with the Flink DataStream and Table API.
...
Follow the [installation](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/get-started/installation/>) instructions to setup Flink and the Flink Agents.
...
Java
  ...
  1. Build Flink Agents from source to generate example jar. See [installation](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/get-started/installation/>) for more details.
