...
A YAML file describes **Resources** and **Actions** declaratively; the **Event** types are referenced by name (built-in event aliases like `input` / `chat_response`, or your own event-type strings). The implementation classes (action functions, tool functions, custom types) still live in your Python or Java code; the YAML file only describes _how_ those pieces are wired together.

If you’d rather declare agents directly in code, see [Workflow Agent](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/development/workflow_agent/>) and [ReAct Agent](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/development/react_agent/>).
...
`{variable_name}` is the only substitution syntax, and there is no `{{` / `}}` escape — write JSON examples in prompt `content` with single braces. See [Brace Handling](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/development/prompts/#brace-handling>).
...
`function` | yes | Fully-qualified callable in the form `<module-or-class>:<qualname>`. See Function references.
`type` | no | Implementation language: `python` or `java`. Defaults to `python` (see Selecting the implementation language).
...
`clazz` | yes | Either a built-in alias (e.g. `ollama`, `openai`) or a fully-qualified class path.
`type` | no | Implementation language: `python` or `java`. Defaults to `python` (see Selecting the implementation language).
_extras_ | no | Any additional keys are forwarded verbatim as `ResourceDescriptor` init arguments (e.g. `base_url`, `endpoint`, `model`, `request_timeout`).
...
**Inter-resource references.** Several fields are not values but _name lookups_ into another section, resolved within the agent first and then against shared resources. In the single-agent example:
...
`function` | yes | Fully-qualified callable in the form `<module-or-class>:<qualname>`. See Function references.
`trigger_conditions` | yes | List of event types the action listens to. Built-in event aliases (`input`, `chat_request`, …) or your own event-type strings.
`type` | no | Implementation language: `python` or `java`. Defaults to `python` (see Selecting the implementation language).
...
For an end-to-end runnable walkthrough that loads a YAML-declared agent and runs it on Flink, see [YAML Agent Quickstart](<//nightlies.apache.org/flink/flink-agents-docs-main/docs/get-started/quickstart/yaml_agent/>).
...
When `type:` resolves to the **opposite** language of the loader, the loader builds a cross-language descriptor that delegates to the other-language implementation — see Cross-language agents.
