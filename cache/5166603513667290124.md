...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/sql/reference/queries/over-agg/>).
...
`OVER` windows are defined on an ordered sequence of rows. Since tables do not have an inherent order, the `ORDER BY` clause is mandatory. For streaming queries, Flink currently supports `OVER` windows that are defined with an ascending [time attribute](<//nightlies.apache.org/flink/flink-docs-master/docs/concepts/sql-table-concepts/time_attributes/>) or ascending non-time attribute. Additional orderings are not supported.
