...
Please refer to the [configuration](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/configuration/overview/>) section for more information.
...
For more information on the different execution modes, see also the corresponding [DataStream API section](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/datastream/execution_mode/>).
...
One must meet the following prerequisites before setting the runtime mode to `BATCH`:
  ...
  * Operators need a sufficient amount of [off-heap memory](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/memory/mem_setup_tm/#managed-memory>) for sorting and other intermediate results.
...
A batch execution has the following implications (among others):
  ...
  * Exchanges between tasks might be blocking according to the [`execution.batch-shuffle-mode`](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/config/#execution-batch-shuffle-mode>). This also means potentially less resource requirements compared to executing the same pipeline in streaming mode.
...
From a Table API’s perspective, converting from and to DataStream API is similar to reading from or writing to a virtual table connector that has been defined using a [`CREATE TABLE` DDL](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/table/sql/create/#create-table>) in SQL.
...
The virtual DataStream table source implements [`SupportsSourceWatermark`](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/table/sourcessinks/#source-abilities>) and thus allows calling the `SOURCE_WATERMARK()` built-in function as a watermark strategy to adopt watermarks from the DataStream API.
...
Since `DataType` is richer than `TypeInformation`, we can easily enable immutable POJOs and other complex data structures. The following example in Java shows what is possible. Check also the [Data Types & Serialization](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/datastream/fault-tolerance/serialization/types_serialization/>) page of the DataStream API for more information about the supported types there.
...
Internally, Flink’s table runtime is a changelog processor. The concepts page describes how [dynamic tables and streams relate](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/table/concepts/dynamic_tables/>) to each other.
...
The virtual table source implements [`SupportsSourceWatermark`](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/table/sourcessinks/#source-abilities>).
...
For more information about which conversions are supported for data types in Example 4, see the [Table API’s Data Types page](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/table/types/>).
...
The following rules apply when converting `TypeInformation` to a `DataType`:
  ...
  * See [ TypeInfoDataTypeConverter ](<//github.com/apache/flink/blob/release-1.18/flink-table/flink-table-common/src/main/java/org/apache/flink/table/types/utils/TypeInfoDataTypeConverter.java>) for the full translation logic.
...
**Note:** A detailed discussion about dynamic tables and their properties is given in the [Dynamic Tables](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/table/concepts/dynamic_tables/>) document.
...
Flink supports POJOs as composite types. The rules for what determines a POJO are documented [here](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/dev/datastream/fault-tolerance/serialization/types_serialization/#pojos>).
