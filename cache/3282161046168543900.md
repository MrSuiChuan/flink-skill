...
When working with _event-time_ windowing, it can happen that elements arrive late, _i.e._ the event time watermark that Flink uses to keep track of the progress of event-time is already past the end timestamp of a window to which an element belongs. See [event time](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/dev/datastream-v2/time-processing/event_timer_service/>) for a more thorough discussion of how Flink deals with event time.
...
In the following example, we first use the [event time extension](<//nightlies.apache.org/flink/flink-docs-release-2.3/docs/dev/datastream-v2/time-processing/event_timer_service/>) to extract the event time from `orderSoucrce`, then declare a one-hour tumbling window and calculate the sales quantity of each product when the window is triggered.
