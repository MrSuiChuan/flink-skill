...
> This documentation is for an unreleased version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/learn-flink/table_api/>).
...
The Table API is Flink’s declarative, relational API. You describe _what_ you want to compute, and Flink figures out _how_ to compute it efficiently. The same queries work on both batch and streaming data without modification, and Flink’s optimizer automatically selects efficient execution plans.
...
Tables can be created from [external systems](<//nightlies.apache.org/flink/flink-docs-master/docs/connectors/table/overview/>) (Kafka, files, databases), from DataStreams, or inline using values.
...
For a complete list of built-in functions, see the [Built-in Functions](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/systemfunctions/>) reference.
...
One of the most important concepts to understand when using the Table API for streaming is _stream-table duality_ : a stream can be viewed as a table (with rows being inserted over time), and a table can be viewed as a changelog stream (each change to the table is an event).

###  Append-Only Tables #

Simple operations like `filter()` and `select()` produce _append-only_ tables. New rows are inserted, but existing rows are never modified or deleted. These produce only `+I` (INSERT) flags.

###  Updating Tables #

Aggregations and other stateful operations produce _updating_ tables. When the aggregated result for a group changes, Flink must retract the old value and insert the new one.
...
For more on windows, see the [Group Aggregation](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/tableapi/#group-windows>) documentation.
...
For more details on UDFs, see the [User-defined Functions](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/udfs/>) documentation.
...
For the complete PTF guide, see [Process Table Functions](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/ptfs/>).

__Back to top

##  Hands-on #

At this point you know enough to get started coding and running a simple Table API application. Clone the [ flink-training-repo ](<//github.com/apache/flink-training/tree/master/>) , and after following the instructions in the README, try the Table API exercises.

__Back to top

##  Further Reading #

  * [Table API Operations](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/tableapi/>) \- Complete reference for all operations
  * [Built-in Functions](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/systemfunctions/>) \- All available functions
  * [User-Defined Functions](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/udfs/>) \- Creating custom functions
  * [Process Table Functions](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/functions/ptfs/>) \- Advanced stateful processing
  * [SQL & Table API Connectors](<//nightlies.apache.org/flink/flink-docs-master/docs/connectors/table/overview/>) \- Reading from and writing to external systems
  * [Common Concepts](<//nightlies.apache.org/flink/flink-docs-master/docs/dev/table/common/>) \- Shared concepts between Table API and SQL
