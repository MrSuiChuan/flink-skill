...
In order to create your own Flink DataStream program, we encourage you to start with anatomy of a Flink Program and gradually add your own [stream transformations](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/operators/overview/>). The remaining sections act as references for additional operations and advanced features.
...
We will now give an overview of each of those steps, please refer to the respective sections for more details. Note that all core classes of the Java DataStream API can be found in [ org.apache.flink.streaming.api ](<//github.com/apache/flink/blob/release-2.0//flink-streaming-java/src/main/java/org/apache/flink/streaming/api>) .
...
Collection-based:
  ...
  * `fromData(Collection)` \- Creates a data stream from the Java Java.util.Collection. All elements in the collection must be of the same type.

  * `fromData(T ...)` \- Creates a data stream from the given sequence of objects. All objects must be of the same type.
...
Custom:
  ...
  * `addSource` \- Attach a new source function. For example, to read from Apache Kafka you can use `addSource(new FlinkKafkaConsumer<>(...))`. See [connectors](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/connectors/datastream/overview/>) for more details.

__Back to top

##  DataStream Transformations #

Please see [operators](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/operators/overview/>) for an overview of the available stream transformations.
...
Data sinks consume DataStreams and forward them to files, sockets, external systems, or print them. Flink comes with a variety of built-in output formats that are encapsulated behind operations on the DataStreams:
  ...
  * `sinkTo(FileSink.forRowFormat(new Path("outputPath"), new SimpleStringEncoder<>()).build())` \- Writes elements line-wise as Strings. The Strings are obtained by calling the _toString()_ method of each element.
...
Please refer to [execution configuration](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/deployment/config/>) for an explanation of most parameters. These parameters pertain specifically to the DataStream API:
...
[State & Checkpointing](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/fault-tolerance/checkpointing/>) describes how to enable and configure Flink’s checkpointing mechanism.
...
##  Where to go next? #
  ...
  * [Operators](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/operators/overview/>): Specification of available streaming operators.
  * [Event Time](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/concepts/time/>): Introduction to Flink’s notion of time.
  * [State & Fault Tolerance](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/dev/datastream/fault-tolerance/state/>): Explanation of how to develop stateful applications.
  * [Connectors](<//nightlies.apache.org/flink/flink-docs-release-2.0/docs/connectors/datastream/overview/>): Description of available input and output connectors.
