...
This connector provides a unified Source and Sink for `BATCH` and `STREAMING` that reads or writes (partitioned) files to file systems supported by the [Flink `FileSystem` abstraction](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/deployment/filesystems/overview/>). This filesystem connector provides the same guarantees for both `BATCH` and `STREAMING` and is designed to provide exactly-once semantics for `STREAMING` execution.

The connector supports reading and writing a set of files from any (distributed) file system (e.g. POSIX, S3, HDFS) with a [format](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/connectors/datastream/formats/overview/>) (e.g., Avro, CSV, Parquet), and produces a stream or records.

##  File Source #

The `File Source` is based on the [Source API](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/datastream/sources/#the-data-source-api>), a unified data source that reads files - both in batch and in streaming mode. It is divided into the following two parts: `SplitEnumerator` and `SourceReader`.
...
You will need to combine the File Source with a [format](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/connectors/datastream/formats/overview/>), which allows you to parse CSV, decode AVRO, or read Parquet columnar files.
...
> If you are interested in how File Source works through the new data source API design, you may want to read this part as a reference. For details about the new data source API, check out the [documentation on data sources](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/datastream/sources/>) and [FLIP-27](<https://cwiki.apache.org/confluence/display/FLINK/FLIP-27%3A+Refactor+Source+Interface>) for more descriptive discussions.
...
Please check out the JavaDoc for [ FileSink ](<//nightlies.apache.org/flink/flink-docs-release-1.20/api/java//org/apache/flink/connector/file/sink/FileSink.html>) for all the configuration options and more documentation about the implementation of the different data formats.
...
Bulk-encoded sinks are created similarly to the row-encoded ones, but instead of specifying an `Encoder`, we have to specify a [ BulkWriter.Factory ](<//nightlies.apache.org/flink/flink-docs-release-1.20/api/java//org/apache/flink/api/common/serialization/BulkWriter.Factory.html>) . The `BulkWriter` logic defines how new elements are added and flushed, and how a batch of records is finalized for further encoding purposes.
...
PyFlink JAR | [Download](<https://repo.maven.apache.org/maven2/org/apache/flink/flink-sql-parquet/1.20.3/flink-sql-parquet-1.20.3.jar>)
---
...
PyFlink JAR | [Download](<https://repo.maven.apache.org/maven2/org/apache/flink/flink-sql-avro/1.20.3/flink-sql-avro-1.20.3.jar>)
---
See [Python dependency management](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/python/dependency_management/#jar-dependencies>) for more details on how to use JARs in PyFlink.
...
PyFlink JAR | [Download](<https://repo.maven.apache.org/maven2/org/apache/flink/flink-sql-orc/1.20.3/flink-sql-orc-1.20.3.jar>)
---
See [Python dependency management](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/dev/python/dependency_management/#jar-dependencies>) for more details on how to use JARs in PyFlink.
...
Since version 1.15 `FileSink` supports compaction of the `pending` files, which allows the application to have smaller checkpoint interval without generating a lot of small files, especially when using the [bulk encoded formats](<//nightlies.apache.org/flink/flink-docs-release-1.20/docs/connectors/datastream/filesystem/#bulk-encoded-formats>) that have to rolling on taking checkpoints.
...
When enabling compaction, you need to specify the [ FileCompactStrategy ](<//nightlies.apache.org/flink/flink-docs-release-1.20/api/java//org/apache/flink/connector/file/sink/compactor/FileCompactStrategy.html>) and the [ FileCompactor ](<//nightlies.apache.org/flink/flink-docs-release-1.20/api/java//org/apache/flink/connector/file/sink/compactor/FileCompactor.html>) .

The [ FileCompactStrategy ](<//nightlies.apache.org/flink/flink-docs-release-1.20/api/java//org/apache/flink/connector/file/sink/compactor/FileCompactStrategy.html>) specifies when and which files get compacted. Currently, there are two parallel conditions: the target file size and the number of checkpoints get passed. Once the total size of the cached files has reached the size threshold or the number of checkpoints since the last compaction has reached the specified number, the cached files will be scheduled to compact.

The [ FileCompactor ](<//nightlies.apache.org/flink/flink-docs-release-1.20/api/java//org/apache/flink/connector/file/sink/compactor/FileCompactor.html>) specifies how to compact the give list of `Path` and write the result file. It could be classified into two types according to how to write the file:

  * **[ OutputStreamBasedFileCompactor ](<//nightlies.apache.org/flink/flink-docs-release-1.20/api/java//org/apache/flink/connector/file/sink/compactor/OutputStreamBasedFileCompactor.html>) **: The users can write the compacted results into an output stream. This is useful when the users don’t want to or can’t read records from the input files. An example is the [ ConcatFileCompactor ](<//nightlies.apache.org/flink/flink-docs-release-1.20/api/java//org/apache/flink/connector/file/sink/compactor/ConcatFileCompactor.html>) that concats the list of files directly.
  * **[ RecordWiseFileCompactor ](<//nightlies.apache.org/flink/flink-docs-release-1.20/api/java//org/apache/flink/connector/file/sink/compactor/RecordWiseFileCompactor.html>) **: The compactor can read records one-by-one from the input files and write into the result file similar to the `FileWriter`. An example is the [ RecordWiseFileCompactor ](<//nightlies.apache.org/flink/flink-docs-release-1.20/api/java//org/apache/flink/connector/file/sink/compactor/RecordWiseFileCompactor.html>) that reads records from the source files and then writes them with the `CompactingFileWriter`. Users need to specify how to read records from the source files.
