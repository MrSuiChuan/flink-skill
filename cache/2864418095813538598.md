...
Currently, it supports two kinds of Python user-defined functions: the [general Python user-defined functions](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/python/table/udfs/python_udfs/>) which process data one row at a time and [vectorized Python user-defined functions](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/python/table/udfs/vectorized_python_udfs/>) which process data one batch at a time.

##  Bundling UDFs #

To run Python UDFs (as well as Pandas UDFs) in any non-local mode, it is strongly recommended bundling your Python UDF definitions using the config option [`python-files`](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/python/python_config/#python-files>), if your Python UDFs live outside the file where the `main()` function is defined. Otherwise, you may run into `ModuleNotFoundError: No module named 'my_udf'` if you define Python UDFs in a file called `my_udf.py`.
