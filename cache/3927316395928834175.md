...
If you want to jump right in, [set up a Flink program](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/project-configuration/>) and add the FlinkCEP dependency to the `pom.xml` of your project.
...
Java
    ...
    &ltdependency&gt
        ...
        &ltartifactId&gtflink-cep_2.11&lt/artifactId&gt
...
Scala
    ...
    &ltdependency&gt
        &ltgroupId&gtorg.apache.flink&lt/groupId&gt
        &ltartifactId&gtflink-cep-scala_2.11&lt/artifactId&gt
        &ltversion&gt1.13.6&lt/version&gt
    &lt/dependency&gt

Copied to clipboard!

FlinkCEP is not part of the binary distribution. See how to link with it for cluster execution [here](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/project-configuration/>).
...
> For `SKIP_TO_FIRST`/`LAST` there are two options how to handle cases when there are no elements mapped to the specified variable. By default a NO_SKIP strategy will be used in this case. The other option is to throw exception in such situation. One can enable this option by:
...
Note The `processTimedOutMatch` does not give one access to the main output. You can still emit results through [side-outputs](<//nightlies.apache.org/flink/flink-docs-release-1.13/docs/dev/datastream/side_output/>) though, through the `Context` object.
