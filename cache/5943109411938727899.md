...
_SQL 客户端_ 的目的是提供一种简单的方式来编写、调试和提交表程序到 Flink 集群上，而无需写一行 Java 或 Scala 代码。 _SQL 客户端命令行界面（CLI）_ 能够在命令行中检索和可视化分布式应用中实时产生的结果。

##  入门 #

本节介绍如何在命令行里启动（setup）和运行你的第一个 Flink SQL 程序。

SQL 客户端捆绑在常规 Flink 发行版中，因此可以直接运行。它仅需要一个正在运行的 Flink 集群就可以在其中执行表程序。有关设置 Flink 群集的更多信息，请参见[集群和部署](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/resource-providers/standalone/overview/>)部分。如果仅想试用 SQL 客户端，也可以使用以下命令启动本地集群：
...
SQL Client 脚本也位于 Flink 的 bin 目录中。用户可以通过启动嵌入式 standalone 进程或通过连接到远程 [SQL Gateway](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/sql/interfaces/sql-gateway/overview/>) 来启动 SQL 客户端命令行界面。SQL 客户端默认使用 `embedded` 模式，你可以通过以下方式启动 CLI：
...
By default, the SQL Client will use the truststore configured using the `security.ssl.rest.truststore` and `security.ssl.rest.truststore-password` properties in the [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/config/#flink-配置文件>) on the SQL client side. If these properties aren’t explicitly configured, the client will use the default certificate stores provided by the JDK.

Note SQL 客户端目前只支持和 REST API 版本大于 v1 的 [REST Endpoint](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/sql/interfaces/sql-gateway/rest/#rest-api>) 通信。
...
_表格模式_ 下，可视化结果表将不断更新，直到表程序以如下内容结束：
...
_Tableau模式_ 下，如果这个查询以流的方式执行，那么将显示以下内容：
...
See also the general [SQL](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/sql/reference/overview/>) documentation.
...
You can configure the SQL Client by setting the options below, or any valid [Flink configuration](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/table/config/>) entry:
...
The **changelog mode** does not materialize results and visualizes the result stream that is produced by a [continuous query](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/concepts/sql-table-concepts/dynamic_tables/#continuous-queries>) consisting of insertions (`+`) and retractions (`-`).
...
The full list of offered SQL JARs can be found on the [connection to external systems page](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/connectors/table/overview/>).

You can refer to the [configuration](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/dev/configuration/connector/>) section for information on how to configure connector and format dependencies.
...
Attention The SQL Client does not track the status of the running Flink job after submission. The CLI process can be shutdown after the submission without affecting the detached query. Flink’s `restart strategy` takes care of the fault-tolerance. Please use the job statements to [monitor the detached query status](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/sql/interfaces/sql-client/#monitoring-job-status>) or [stop the detached query](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/sql/interfaces/sql-client/#terminating-a-job>).
...
For more details about creating and managing savepoints, please refer to [Job Lifecycle Management](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/cli/#job-lifecycle-management>).
...
The savepoint path could be specified with [execution.checkpointing.savepoint-dir](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/deployment/config/#state-savepoints-dir>) either in the cluster configuration or session configuration (the latter would take precedence).

For more details about stopping jobs, please refer to [Job Statements](<//nightlies.apache.org/flink/flink-docs-release-2.3/zh/docs/sql/reference/utility/job/#stop-job>).
