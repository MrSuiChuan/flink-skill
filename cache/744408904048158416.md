...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/configuration/advanced/>).
...
有关类加载的更多细节，请查阅 [Flink 类加载](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/ops/debugging/debugging_classloading/>)。
...
有关如何为特定 Scala 版本构建 Flink 的细节，请查阅[构建指南](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/flinkdev/building/#scala-versions>)。

2.12.8 之后的 Scala 版本与之前的 2.12.x 版本二进制不兼容，使 Flink 项目无法将其 2.12.x 版本直接升级到 2.12.8 以上。您可以按照[构建指南](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/flinkdev/building/#scala-versions>)在本地为更高版本的 Scala 构建 Flink 。为此，您需要在构建时添加 `-Djapicmp.skip` 以跳过二进制兼容性检查。
...
Flink 发行版默认包含执行 Flink SQL 任务的必要 JAR 文件（位于 `/lib` 目录），主要有：
  ...
  * `flink-table-api-java-uber-1.20.3.jar` → 包含所有的 Java API；
  * `flink-table-runtime-1.20.3.jar` → 包含 Table 运行时;
  * `flink-table-planner-loader-1.20.3.jar` → 包含查询计划器。

> 以前，这些 JAR 都打包进了 `flink-table.jar`，自从 Flink 1.15 开始，已将其划分成三个 JAR，以允许用户使用 `flink-table-planner-loader-1.20.3.jar` 充当 `flink-table-planner_2.12-1.20.3.jar`。
...
有关更多细节，请查阅如何[连接外部系统](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/connectors/table/overview/>)。

###  Table Planner 和 Table Planner 加载器 #

从 Flink 1.15 开始，发行版包含两个 planner:

  * `flink-table-planner_2.12-1.20.3.jar`, 位于 `/opt` 目录, 包含查询计划器；
  * `flink-table-planner-loader-1.20.3.jar`, 位于 `/lib` 目录默认被加载, 包含隐藏在单独的 classpath 里的查询计划器 (您无法直接使用 `io.apache.flink.table.planner` 包)。
...
如果您在 IDE 内开发或测试期间需要 Hadoop 依赖项（比如用于 HDFS 访问），应该限定这些依赖项的使用范围（如 _test_ 或 _provided_ ）。