...
A Temporal table function provides access to the version of a temporal table at a specific point in time. In order to access the data in a temporal table, one must pass a [time attribute](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/dev/table/concepts/time_attributes/>) that determines the version of the table that will be returned. Flink uses the SQL syntax of [table functions](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/dev/table/functions/udfs/#table-functions>) to provide a way to express it.
...
Temporal table functions can be defined on top of append-only streams using the [Table API](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/dev/table/tableapi/>). The table is registered with one or more key columns, and a time attribute used for versioning.
