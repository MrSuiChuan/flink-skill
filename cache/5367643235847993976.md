#  [PyFlink: The integration of Pandas into PyFlink](</2020/08/04/pyflink-the-integration-of-pandas-into-pyflink/>)

August 4, 2020 - Jincheng Sun [(@sunjincheng121)](<https://twitter.com/sunjincheng121>) Markos Sfikas [(@MarkSfik)](<https://twitter.com/MarkSfik>)
...
[Pic source: VanderPlas 2017, slide 52.](<https://speakerdeck.com/jakevdp/the-unexpected-effectiveness-of-python-in-science?slide=52>)
...
Note Please note that Python 3.5 or higher is required to install and run PyFlink
...
The Pandas UDF above uses the Pandas `dataframe.interpolate()` function to interpolate the missing temperature data for each equipment id. This is a common IoT scenario whereby each equipment/device reports it’s id and temperature to be analyzed, but the temperature field may be null due to various reasons. With the function, you can register and use it in the same way as the [normal Python UDF](<https://flink.apache.org/2020/04/09/pyflink-udf-support-flink.html>). Below is a complete example of how to use the Pandas UDF in PyFlink.
...
The command builds and runs the Python Table API program in a local mini-cluster. You can also submit the Python Table API program to a remote cluster using different command lines, see more details [here](<//nightlies.apache.org/flink/flink-docs-release-1.11/ops/cli.html#job-submission-examples>).
...
[ Edit This Page __](<//github.com/apache/flink-web/edit/asf-site/docs/content/posts/2020-08-04-pyflink-pandas-udf-support-flink.md>)