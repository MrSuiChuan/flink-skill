...
If you want to jump right in, [set up a Flink program](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/configuration/overview/>) and add the FlinkCEP dependency to the `pom.xml` of your project.
...
FlinkCEP is not part of the binary distribution. See how to link with it for cluster execution [here](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/configuration/overview/>).
...
Note The `processTimedOutMatch` does not give one access to the main output. You can still emit results through [side-outputs](<//nightlies.apache.org/flink/flink-docs-release-1.17/docs/dev/datastream/side_output/>) though, through the `Context` object.
