...
The command starts the SQL Gateway with REST Endpoint that listens on the address localhost:8083. You can use the curl command to check whether the REST Endpoint is available.
    ...
    {"productName":"Apache Flink","version":"2.1.1"}
...
SQL Gateway supports deploying a script in [Application Mode](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/deployment/overview/>). In application mode, [JobManager](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/concepts/flink-architecture/#jobmanager>) is responsible for compiling the script. If you want to use custom resources in the script, e.g. Kafka Source, please use [ADD JAR](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/dev/table/sql/jar/>) command to download the [required artifacts](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/dev/configuration/connector/#available-artifacts>).
...
Note If you want to run the script with PyFlink, please use an image with PyFlink installed. You can refer to [Enabling PyFlink in docker](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/deployment/resource-providers/standalone/docker/#enabling-python>) for more details.
...
You can configure the SQL Gateway when starting the SQL Gateway below, or any valid [Flink configuration](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/dev/table/config/>) entry:
...
Flink natively supports [REST Endpoint](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/dev/table/sql-gateway/rest/>) and [HiveServer2 Endpoint](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/dev/table/sql-gateway/hiveserver2/>). The SQL Gateway is bundled with the REST Endpoint by default. With the flexible architecture, users are able to start the SQL Gateway with the specified endpoints by calling


    $ ./bin/sql-gateway.sh start -Dsql-gateway.endpoint.type=hiveserver2


or add the following config in the [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/deployment/config/#flink-configuration-file>):


    sql-gateway.endpoint.type: hiveserver2


> Notice: The CLI command has higher priority if [Flink configuration file](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/deployment/config/#flink-configuration-file>) also contains the option `sql-gateway.endpoint.type`.
