...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/table/data_stream_api/>).
...
Both APIs can work with bounded _and_ unbounded streams.
...
The following example shows how updating tables can be converted. Every result row represents an entry in a changelog with a change flag that can be queried by calling `row.getKind()` on it. In the example, the second score for `Alice` creates an _update before_ (`-U`) and _update after_ (`+U`) change.
...
The example above shows how the final result is computed incrementally by continuously emitting [row-wise updates](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/concepts/dynamic_tables/>) for each incoming record. However, in cases where the input streams are finite (i.e. _bounded_), a result can be computed more efficiently by leveraging batch processing principles.

In batch processing, operators can be executed in successive stages that consume the entire input table before emitting results. For example, a join operator can sort both bounded inputs before performing the actual joining (i.e. _sort-merge join algorithm_), or build a hash table from one input before consuming the other (i.e. build/probe phase of the _hash join algorithm_).

Both DataStream API and Table API offer a specialized _batch runtime mode_.
...
请查阅[配置](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/configuration/overview/>)小节了解更多细节。
...
The DataStream API’s `StreamExecutionEnvironment` uses a _builder pattern_ to construct a complex pipeline. The pipeline possibly splits into multiple branches that might or might not end with a sink. The environment buffers all these defined branches until it comes to job submission.
...
The _batch runtime mode_ is a specialized execution mode for _bounded_ Flink programs.

Generally speaking, _boundedness_ is a property of a data source that tells us whether all the records coming from that source are known before execution or whether new data will show up, potentially indefinitely. A job, in turn, is bounded if all its sources are bounded, and unbounded otherwise.

_Streaming runtime mode_ , on the other hand, can be used for both bounded and unbounded jobs.

For more information on the different execution modes, see also the corresponding [DataStream API section](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/datastream/execution_mode/>).
...
One must meet the following prerequisites before setting the runtime mode to `BATCH`:
  ...
  * Operators need a sufficient amount of [off-heap memory](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/memory/mem_setup_tm/#managed-memory>) for sorting and other intermediate results.
...
A batch execution has the following implications (among others):
  ...
  * Exchanges between tasks might be blocking according to the [`execution.batch-shuffle-mode`](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#execution-batch-shuffle-mode>). This also means potentially less resource requirements compared to executing the same pipeline in streaming mode.
...
> Since batch processing can be considered as _a special case of stream processing_ , we recommend implementing a streaming pipeline first as it is the most general implementation for both bounded and unbounded data.
>
...
The following example shows how to play around with batch mode using the [DataGen table source](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/connectors/table/datagen/>). Many sources offer options that implicitly make the connector bounded, for example, by defining a terminating offset or timestamp. In our example, we limit the number of rows with the `number-of-rows` option.
...
From a Table API’s perspective, converting from and to DataStream API is similar to reading from or writing to a virtual table connector that has been defined using a [`CREATE TABLE` DDL](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sql/create/#create-table>) in SQL.
...
The virtual DataStream table source implements [`SupportsSourceWatermark`](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sourcessinks/#source-abilities>) and thus allows calling the `SOURCE_WATERMARK()` built-in function as a watermark strategy to adopt watermarks from the DataStream API.
...
Since `DataType` is richer than `TypeInformation`, we can easily enable immutable POJOs and other complex data structures. The following example in Java shows what is possible. Check also the [Data Types & Serialization](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/datastream/fault-tolerance/serialization/types_serialization/>) page of the DataStream API for more information about the supported types there.
...
> Views created from a `DataStream` can only be registered as temporary views. Due to their _inline_ /_anonymous_ nature, it is not possible to register them in a permanent catalog.
...
Internally, Flink’s table runtime is a changelog processor. The concepts page describes how [dynamic tables and streams relate](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/concepts/dynamic_tables/>) to each other.
...
A `StreamTableEnvironment` offers the following methods to expose these _change data capture_ (CDC) functionalities:
  ...
  * `fromChangelogStream(DataStream, Schema, ChangelogMode)`: Gives full control about how to interpret a stream as a changelog. The passed `ChangelogMode` helps the planner to distinguish between _insert-only_ , _upsert_ , or _retract_ behavior.
  ...
  * `toChangelogStream(Table, Schema, ChangelogMode)`: Gives full control about how to convert a table to a changelog stream. The passed `ChangelogMode` helps the planner to distinguish between _insert-only_ , _upsert_ , or _retract_ behavior.
...
The virtual table source implements [`SupportsSourceWatermark`](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/sourcessinks/#source-abilities>).
...
For more information about which conversions are supported for data types in Example 4, see the [Table API’s Data Types page](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/types/>).
...
Source-to-sink pipelines defined in Table API can be attached _as a whole_ to the `StreamExecutionEnvironment` and will be submitted when calling one of the `execute` methods in the DataStream API.
...
Users of the Scala API can use all the conversion methods above in a more fluent way by leveraging Scala’s _implicit_ feature.
...
> The DataStream API is not always able to extract a more specific `TypeInformation` based on [reflection](<https://www.oracle.com/technical-resources/articles/java/javareflection.html>). This often happens silently and leads to `GenericTypeInfo` that is backed by the generic Kryo serializer.
>
...
The following rules apply when converting `TypeInformation` to a `DataType`:
  ...
  * See [ TypeInfoDataTypeConverter ](<//github.com/apache/flink/blob/release-1.20/flink-table/flink-table-common/src/main/java/org/apache/flink/table/types/utils/TypeInfoDataTypeConverter.java>) for the full translation logic.
...
**注意：** 文档[动态表](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/concepts/dynamic_tables/>)给出了有关动态表及其属性的详细讨论。
...
基于位置的映射可在保持字段顺序的同时为字段提供更有意义的名称。这种映射方式可用于 _具有特定的字段顺序_ 的复合数据类型以及原子类型。如 tuple、row 以及 case class 这些复合数据类型都有这样的字段顺序。然而，POJO 类型的字段则必须通过名称映射（参见下一章）。可以将字段投影出来，但不能使用`as`(Java 和 Scala) 或者 `alias`(Python)重命名。
...
Flink 支持 Scala 的内置 tuple 类型并给 Java 提供自己的 tuple 类型。 两种 tuple 的 DataStream 都能被转换成表。 可以通过提供所有字段名称来重命名字段（基于位置映射）。 如果没有指明任何字段名称，则会使用默认的字段名称。 如果引用了原始字段名称（对于 Flink tuple 为`f0`、`f1` … …，对于 Scala tuple 为`_1`、`_2` … …），则 API 会假定映射是基于名称的而不是基于位置的。 基于名称的映射可以通过 `as` 对字段和投影进行重新排序。

Java

Flink 给 Java 提供自己的 tuple 类型。 tuple 的 DataStream 都能被转换成表。 可以通过提供所有字段名称来重命名字段（基于位置映射）。 如果没有指明任何字段名称，则会使用默认的字段名称。 如果引用了原始字段名称（对于 Flink tuple 为`f0`、`f1` … …），则 API 会假定映射是基于名称的而不是基于位置的。 基于名称的映射可以通过 `as` 对字段和投影进行重新排序。
...
Flink 支持 Scala 的内置 tuple 类型。 tuple 的 DataStream 都能被转换成表。 可以通过提供所有字段名称来重命名字段（基于位置映射）。 如果没有指明任何字段名称，则会使用默认的字段名称。 如果引用了原始字段名称（对于 Scala tuple 为`_1`、`_2` … …），则 API 会假定映射是基于名称的而不是基于位置的。 基于名称的映射可以通过 `as` 对字段和投影进行重新排序。
...
Flink 支持 Python 的内置 tuple 类型。 tuple 的 DataStream 都能被转换成表。 可以通过提供所有字段名称来重命名字段（基于位置映射）。 如果没有指明任何字段名称，则会使用默认的字段名称。 如果引用了原始字段名称（对于 Python tuple 为`f0`、`f1` … …），则 API 会假定映射是基于名称的而不是基于位置的。 基于名称的映射可以通过 `alias` 对字段和投影进行重新排序。
...
Flink 支持 POJO 类型作为复合类型。确定 POJO 类型的规则记录在[这里](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/datastream/fault-tolerance/serialization/types_serialization/#pojos>).
