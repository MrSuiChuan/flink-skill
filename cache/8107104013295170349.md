...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/dev/table/timezone/>).

#  时区 #

Flink 为日期和时间提供了丰富的数据类型， 包括 `DATE`， `TIME`， `TIMESTAMP`， `TIMESTAMP_LTZ`， `INTERVAL YEAR TO MONTH`， `INTERVAL DAY TO SECOND` (更多详情请参考 [Date and Time](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/types/#date-and-time>))。 Flink 支持在 session （会话）级别设置时区（更多详情请参考 [table.local-time-zone](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/config/#table-local-time-zone>)）。 Flink 对多种时间类型和时区的支持使得跨时区的数据处理变得非常容易。
...
更多时间属性相关的详细介绍， 请参考 [Time Attribute](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/dev/table/concepts/time_attributes/#%e6%97%b6%e9%97%b4%e5%b1%9e%e6%80%a7>) 。
