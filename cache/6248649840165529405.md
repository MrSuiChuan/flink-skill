...
There are multiple ways to install Flink. For experimentation, the most common option is to download the binaries and run them locally. You can follow the steps in [local installation](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/try-flink/local_installation/>) to set up an environment for the rest of the tutorial.
...
The [SQL Client](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sqlclient/>) is an interactive client to submit SQL queries to Flink and visualize the results. To start the SQL client, run the `sql-client` script from the installation folder.
...
Running the `HELP` command lists the full set of supported SQL statements. Let’s run one such command, `SHOW`, to see a full list of Flink [built-in functions](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/functions/systemfunctions/>).
...
Tables can be defined through the SQL client or using environment config file. The SQL client support [SQL DDL commands](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sql/overview/>) similar to traditional SQL. Standard SQL DDL is used to [create](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sql/create/>), [alter](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sql/alter/>), [drop](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sql/drop/>) tables.

Flink has a support for different [connectors](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/connectors/table/overview/>) and [formats](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/connectors/table/formats/overview/>) that can be used with tables. Following is an example to define a source table backed by a [CSV file](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/connectors/table/formats/csv/>) with `emp_id`, `name`, `dept_id` as columns in a `CREATE` table statement.
...
A [continuous query](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/concepts/dynamic_tables/#continuous-queries>) never terminates and produces a dynamic table as a result. [Dynamic tables](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/concepts/dynamic_tables/#continuous-queries>) are the core concept of Flink’s Table API and SQL support for streaming data.
...
##  Resources to Learn more #
  ...
  * [SQL](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sql/overview/>): Supported operations and syntax for SQL.
  * [SQL Client](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/sqlclient/>): Play around with Flink SQL and submit a table program to a cluster without programming knowledge
  * [Concepts & Common API](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/common/>): Shared concepts and APIs of the Table API and SQL.
  * [Streaming Concepts](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/concepts/overview/>): Streaming-specific documentation for the Table API or SQL such as configuration of time attributes and handling of updating results.
  * [Built-in Functions](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/functions/systemfunctions/>): Supported functions in Table API and SQL.
  * [Connect to External Systems](<//nightlies.apache.org/flink/flink-docs-release-2.2/docs/connectors/table/overview/>): Available connectors and formats for reading and writing data to external systems.
