...
A system-wide default parallelism for all execution environments can be defined by setting the `parallelism.default` property in `./conf/flink-conf.yaml`. See the [Configuration](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/deployment/config/>) documentation for details.
