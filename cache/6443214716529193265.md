...
Note that these examples are _not_ exhaustive and you can use S3 in other places as well, including your [high availability setup](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/deployment/ha/overview/>) or the [EmbeddedRocksDBStateBackend](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/ops/state/state_backends/#the-rocksdbstatebackend>); everywhere that Flink expects a FileSystem URI (unless otherwise stated).
...
It is the only S3 file system with support for the [FileSystem](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/connectors/datastream/filesystem/>).

Both `flink-s3-fs-hadoop` and `flink-s3-fs-presto` register default FileSystem wrappers for URIs with the _s3://_ scheme, `flink-s3-fs-hadoop` also registers for _s3a://_ and `flink-s3-fs-presto` also registers for _s3p://_ , so you can use this to use both at the same time. For example, the job uses the [FileSystem](<//nightlies.apache.org/flink/flink-docs-release-1.18/docs/connectors/datastream/filesystem/>) which only supports Hadoop, but uses Presto for checkpointing. In this case, you should explicitly use _s3a://_ as a scheme for the sink (Hadoop) and _s3p://_ for checkpointing (Presto).
...
To use `flink-s3-fs-hadoop` or `flink-s3-fs-presto`, copy the respective JAR file from the `opt` directory to the `plugins` directory of your Flink distribution before starting Flink, e.g.
    ...
    cp ./opt/flink-s3-fs-presto-1.18.1.jar ./plugins/s3-fs-presto/
