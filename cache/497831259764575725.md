...
In this section you will learn about the APIs that Flink provides for writing stateful programs. Please take a look at [Stateful Stream Processing](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/concepts/stateful-stream-processing/>) to learn about the concepts behind stateful stream processing.
...
State is accessed using the `RuntimeContext`, so it is only possible in _rich functions_. Please see [here](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/dev/datastream/user_defined_functions/#rich-functions>) for information about that, but we will also see an example shortly. The `RuntimeContext` that is available in a `RichFunction` has these methods for accessing state:
...
_Operator State_ (or _non-keyed state_) is state that is bound to one parallel operator instance. The [Kafka Connector](<//nightlies.apache.org/flink/flink-docs-release-1.14/docs/connectors/datastream/kafka/>) is a good motivating example for the use of Operator State in Flink. Each parallel instance of the Kafka consumer maintains a map of topic partitions and offsets as its Operator State.
