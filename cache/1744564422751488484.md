...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/zh/docs/deployment/security/security-kerberos/>).
...
该模块使用 Hadoop UserGroupInformation（UGI）类来建立进程范围的 _登录用户_ 上下文。然后，登录用户用于与 Hadoop 组件的所有交互，包括 HDFS、HBase 和 YARN。
...
在 standalone 模式或集群模式下运行安全 Flink 集群的步骤如下：
  ...
  1. 将与安全相关的配置选项添加到 Flink 配置文件（在所有集群节点上执行）（详见[此处](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#auth-with-external-systems>)）。
...
在原生 Kubernetes 或 YARN 模式下运行安全 Flink 集群的步骤如下：
  ...
  1. 在客户端的 Flink 配置文件中添加安全相关的配置选项（详见[此处](<//nightlies.apache.org/flink/flink-docs-release-1.20/zh/docs/deployment/config/#auth-with-external-systems>)）。
...
请参阅 [YARN security](<https://github.com/apache/hadoop/blob/trunk/hadoop-yarn-project/hadoop-yarn/hadoop-yarn-site/src/site/markdown/YarnApplicationSecurity.md>) 文档获取更多相关信息。
