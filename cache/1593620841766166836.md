...
If you are looking for pre-defined source connectors, please check the [Connector Docs](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/connectors/datastream/overview/>).
...
The [ Source ](<//github.com/apache/flink/blob/release-2.1/flink-core/src/main/java/org/apache/flink/api/connector/source/Source.java>) class is the API entry point that ties the above three components together.
...
The [ Source ](<//github.com/apache/flink/blob/release-2.1/flink-core/src/main/java/org/apache/flink/api/connector/source/Source.java>) API is a factory style interface to create the following components.
...
In addition to that, the Source provides the [ boundedness ](<//github.com/apache/flink/blob/release-2.1/flink-core/src/main/java/org/apache/flink/api/connector/source/Boundedness.java>) attribute of the source, so that Flink can choose the appropriate mode to run the Flink jobs.
...
The [ SplitEnumerator ](<//github.com/apache/flink/blob/release-2.1/flink-core/src/main/java/org/apache/flink/api/connector/source/SplitEnumerator.java>) is expected to be the “brain” of the Source. Typical implementations of the `SplitEnumerator` do the following:
...
A `SplitEnumerator` can accomplish the above work with the help of the [ SplitEnumeratorContext ](<//github.com/apache/flink/blob/release-2.1/flink-core/src/main/java/org/apache/flink/api/connector/source/SplitEnumeratorContext.java>) which is provided to the `Source` on creation or restore of the `SplitEnumerator`. The `SplitEnumeratorContext` allows a `SplitEnumerator` to retrieve necessary information of the readers and perform coordination actions. The `Source` implementation is expected to pass the `SplitEnumeratorContext` to the `SplitEnumerator` instance.
...
The [ SourceReader ](<//github.com/apache/flink/blob/release-2.1/flink-core/src/main/java/org/apache/flink/api/connector/source/SourceReader.java>) is a component running in the Task Managers to consume the records from the Splits.
...
The `SplitReader` API only has three methods:
  ...
  * A blocking fetch method to return a [ RecordsWithSplitIds ](<//github.com/apache/flink/blob/release-2.1/flink-connectors/flink-connector-base/src/main/java/org/apache/flink/connector/base/source/reader/RecordsWithSplitIds.java>)
...
_Event Time_ assignment and _Watermark Generation_ happen as part of the data sources. The event streams leaving the Source Readers have event timestamps and (during streaming execution) contain watermarks. See [Timely Stream Processing](<//nightlies.apache.org/flink/flink-docs-release-2.1/docs/concepts/time/>) for an introduction to Event Time and Watermarks.

####  API #

The `WatermarkStrategy` is passed to the Source during creation in the DataStream API and creates both the [ TimestampAssigner ](<//github.com/apache/flink/blob/release-2.1/flink-core/src/main/java/org/apache/flink/api/common/eventtime/TimestampAssigner.java>) and [ WatermarkGenerator ](<//github.com/apache/flink/blob/release-2.1/flink-core/src/main/java/org/apache/flink/api/common/eventtime/WatermarkGenerator.java>) .
