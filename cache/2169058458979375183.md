...
Flink allows reporting metrics to external systems. For more information about Flink’s metric system go to the [metric system documentation](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/ops/metrics/>).
...
**Important:** The jar containing the reporter must be accessible when Flink is started. Reporters that support the `factory.class` property can be loaded as [plugins](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/deployment/filesystems/plugins/>). Otherwise the jar must be placed in the /lib folder. Reporters that are shipped with Flink (i.e., all reporters documented on this page) are available by default.
...
In order to use this reporter you must copy `/opt/flink-metrics-influxdb-1.15.4.jar` into the `plugins/influxdb` folder of your Flink distribution.
...
The reporter would send metrics using http protocol to the InfluxDB server with the specified retention policy (or the default policy specified on the server). All Flink metrics variables (see [List of all Variables](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/ops/metrics/#list-of-all-variables>)) are exported as InfluxDB tags.
...
All Flink metrics variables (see [List of all Variables](<//nightlies.apache.org/flink/flink-docs-release-1.15/docs/ops/metrics/#list-of-all-variables>)) are exported to Prometheus as labels.
