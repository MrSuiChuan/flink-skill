...
> This documentation is for an out-of-date version of Apache Flink. We recommend you use the latest [stable version](<//nightlies.apache.org/flink/flink-docs-stable/docs/dev/datastream/execution/execution_configuration/>).

#  Execution Configuration #

The `StreamExecutionEnvironment` contains the `ExecutionConfig` which allows to set job specific configuration values for the runtime. To change the defaults that affect all jobs, see [Configuration](<//nightlies.apache.org/flink/flink-docs-release-1.19/docs/deployment/config/>).
